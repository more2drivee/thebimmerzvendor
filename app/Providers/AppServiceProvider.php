...
use Modules\Repair\Http\Livewire\ProgressTracker;
use App\System;
...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        Livewire::component('progress-tracker', ProgressTracker::class);
...
$notification_settings = Cache::rememberForever('notification_settings_1', function () {
    $settings = System::where('key', 'notification_settings_1')->value('value');
    return $settings ? json_decode($settings, true) : [];
});

if (!session()->has('notification_settings')) {
    session(['notification_settings' => $notification_settings]);
}
View::share('notification_settings', $notification_settings);
if (!session()->has('notification_settings')) {
    session(['notification_settings' => $notification_settings]);
}
...
View::share('notification_settings', $notification_settings);
        ...
        View::composer(
            ...
            function ($view) {
                if (isAppInstalled()) {
                    $__system_settings = cache()->remember('system_settings_additional', 86400, function () {
                        $keys = ['additional_js', 'additional_css'];
                        $settings = System::getProperties($keys, true);
                        ...
                        foreach ($module_additional_script as $key => $value) {
                            if (! empty($value['additional_js'])) {
                                $settings['additional_js'] = ($settings['additional_js'] ?? '') . $value['additional_js'];
                            }
                            if (! empty($value['additional_css'])) {
                                $settings['additional_css'] = ($settings['additional_css'] ?? '') . $value['additional_css'];
                            }
                        }
