...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('user.view') || auth()->user()->can('user.create') || auth()->user()->can('roles.view')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                route('reports.activity_logs'),
                                __('lang_v1.activity_log'),
                                ['icon' => '', 'active' => request()->segment(1) == 'reports' && request()->segment(2) == 'activity-log']
                            );
                        }
            ...
            //Blog
            $menu->url(action([\App\Http\Controllers\BlogController::class, 'index']), __('blog.blogs'), ['icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="tw-size-5 tw-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"></path><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"></path></svg>', 'active' => request()->segment(1) == 'blogs'])->order(12);
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('customer.view') || auth()->user()->can('customer.view_own')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ContactController::class, 'loyaltyRequestsIndex']),
                                __('lang_v1.loyalty_requests'),
                                ['icon' => '', 'active' => request()->segment(1) == 'contacts' && request()->segment(2) == 'loyalty-requests']
                            );
                        }
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.view') || auth()->user()->can('product.create')) {
                            $sub->url(
                                action([\App\Http\Controllers\ProductManagementController::class, 'index']),
                                __('product_management.title'),
                                ['icon' => '', 'active' => request()->segment(1) == 'product-management']
                            );
                        }
                        ...
                        if (auth()->user()->can('unit.view') || auth()->user()->can('unit.create') ||
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ProductOrganizationController::class, 'index']),
                                __('product_organization.title'),
                                ['icon' => '', 'active' => request()->segment(1) == 'product-organization']
                            );
                        }
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.update')) {
                            ...
                            $sub->url(
                                action([MaintenanceNoteController::class, 'index']),
                                __('repair::lang.Purchase_Requests'),
                                ['icon' => '', 'active' => request()->segment(1) == 'repair' && request()->segment(2) == 'Purchase_Requests']
                            );
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        // Bundle sales (transactions that include bundles)
                        $sub->url(
                            action([\App\Http\Controllers\SellController::class, 'bundleIndex']),
                            __('bundles.overview.transactions_table'),
                            ['icon' => '', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'bundles']
                        );
           ...
           // Accounts dropdown
            $menu->dropdown(
                ...
                function ($sub) use ($enabled_modules) {
                    ...
                    // All account-related items require account.access
                    ...
                    // Expense has its own permission, independent of account.access
                    if (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) {
                        $sub->url(
                            '/expenses',
                            __('expense.expenses'),
                            ['icon' => '', 'active' => request()->segment(1) == 'expenses']
                        );
                    }
                },
                [
            ...
            //Settings Dropdown
            $isAdmin = auth()->user()->hasRole('Admin#' . auth()->user()->business_id) || auth()->user()->can('superadmin');
            ...
            if ($isAdmin ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $isAdmin) {
                        if ($isAdmin && auth()->user()->can('business_settings.access')) {
                            $sub->url(
                        ...
                        if ($isAdmin && auth()->user()->can('invoice_settings.access')) {
                            $sub->url(
                        ...
                        if ($isAdmin && auth()->user()->can('barcode_settings.access')) {
                            $sub->url(
                        ...
                        if ($isAdmin && auth()->user()->can('access_printers')) {
                            $sub->url(
                        ...
                        // if ($isAdmin && (auth()->user()->can('tax_rate.view') || auth()->user()->can('tax_rate.create'))) {
                        //     $sub->url(
                        ...
                        if ($isAdmin && (auth()->user()->can('user.view') || auth()->user()->can('user.create'))) {
                            $sub->url('/roles',
                            __('user.roles'),
                                ['icon' => '', 'active' => request()->segment(1) == 'roles']
                            );
                        ...
                        // if ($isAdmin) {
                        //     $sub->url(
                        //         action([\App\Http\Controllers\BlogController::class, 'index']),
                        //         __('blog.blogs'),
                        //         ['icon' => '', 'active' => request()->segment(1) == 'blogs']
                        //     );
                        // }

                        // if ($isAdmin && in_array('tables', $enabled_modules) && auth()->user()->can('access_tables')) {
                        //     $sub->url(
                        ...
                        // if ($isAdmin && in_array('modifiers', $enabled_modules) && (auth()->user()->can('product.view') || auth()->user()->can('product.create'))) {
                        //     $sub->url(
                    ...
                    $menu->dropdown(
                        __('home.Artificial_Intelligence'),
                        function ($sub) use ($enabled_modules) {

                            $sub->url(
                                '/artificialintelligence/providers',
                                __('home.Providers'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );
                            $sub->url(
                                '/artificialintelligence/diagnose-message/edit',
                                __('home.Diagnose_Message'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );
                            $sub->url(
                                '/artificialintelligence/brandImport',
                                __('home.Fill_Vehicle_Data'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );

                            $sub->url(
                                '/artificialintelligence/settings',
                                __('home.AI_Settings'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );
                            ...
                            $sub->url(
                                '/data-permissions',
                                __('home.data_permissions'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );
                            $sub->url(
                                '/api-settings',
                                __('home.api_settings'),
                                ['icon' => '', 'active' => request()->segment(1) == 'AIIIIIII']
                            );
                        },
                        [
                            'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path d="M10.325 4.317c.426 -1.756 2.924 -1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543 -.94 3.31 .826 2.37 2.37a1.">
                                <path d="M9 12a3 3 0 1 0 6 0a3 3 0 0 0 -6 0"></path>
                            </svg>',
                            'id' => 'tour_step3'
                        ]
                    ...
                    $menu->dropdown(
                        ...
                        function ($sub) use ($enabled_modules) {
                            ...
                            $sub->url(
                                '/treasury',
                                __('treasury::lang.dashboard'),
                                ['icon' => '', 'active' => request()->segment(1) == 'treasury' && request()->segment(2) == '']
                            );
                            $sub->url(
                                '/treasury/create',
                                __('treasury::lang.add_transaction'),
                                ['icon' => '', 'active' => request()->segment(1) == 'treasury' && request()->segment(2) == 'create']
                            );
                    ...
                    // CheckCar - Advanced Car Inspection wizard
                    $menu->url(
                        '/checkcar/inspections/create',
                        __('checkcar::lang.menu_check_car'),
                        [
                            'icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path d="M3 13l2 -5a2 2 0 0 1 2 -1h10a2 2 0 0 1 2 1l2 5"></path>
                                <path d="M5 16h14"></path>
                                <path d="M6.5 20a1.5 1.5 0 1 0 0 -3a1.5 1.5 0 0 0 0 3z"></path>
                                <path d="M17.5 20a1.5 1.5 0 1 0 0 -3a1.5 1.5 0 0 0 0 3z"></path>
                            </svg>',
                            'active' => request()->segment(1) == 'checkcar'
                        ]
                    )->order(105);
