...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $social_login_settings = $this->getSocialLoginSettings($business_id);

        // Notification settings
        $notification_settings = $this->getNotificationSettings($business_id);

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'version_settings', 'social_login_settings', 'notification_settings'));
    }
    ...
    {
        // System::getProperty caches only module version keys; fetch directly instead
        $settings = System::where('key', 'social_login_settings_' . $business_id)->value('value');
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            //upload jobsheet image
            $jobsheet_image_name = $this->businessUtil->uploadFile($request, 'jobsheet_image', 'business_logos', 'image');
            if (! empty($jobsheet_image_name)) {
                $repair_jobsheet_settings = !empty($business->repair_jobsheet_settings) ? json_decode($business->repair_jobsheet_settings, true) : [];
                $repair_jobsheet_settings['jobsheet_image'] = $jobsheet_image_name;
                $business_details['repair_jobsheet_settings'] = json_encode($repair_jobsheet_settings);
            }
            ...
            if (empty($business_details['logo'])) {
                // If no new logo uploaded, remove logo from business_details to preserve existing logo
            ...
            $this->saveSocialLoginSettings($request, $business_id);

            // Save notification settings
            $this->saveNotificationSettings($request, $business_id);
    ...
    public function sendSmsToUsers($number, $url)
    {
        // dd($number);
        try {
    ...
    public function testSmsConfiguration(Request $request)
    {
        try {
            ...
            if (!isset($sms_settings['test_number']) || empty($sms_settings['test_number'])) {
                return response()->json(['success' => false, 'message' => 'Test number is required'], 400);
            }

            if (!isset($sms_settings['url']) || empty($sms_settings['url'])) {
            ...
            Log::info("Attempting to send SMS to: $mobile_number with message: $sms_body");
    ...
    public function sendInfoJoborderAsSms($job_order_id)
    {
        // Explicitly select contact_id from contacts to avoid ambiguity
        $info = DB::table('repair_job_sheets')
            ...
            ->where('repair_job_sheets.id', $job_order_id)
            ->select(
                'repair_job_sheets.id as job_sheet_id',
                'bookings.contact_id as booking_contact_id',
        ...
        $welcome = 'اهلا ا/' . $info->first_name . ' تابع معلومات طلبك من هنا ';

        // Get customerAppDomain from database (common_settings)
        // Note: common_settings is already cast to array in Business model
        $business = Business::first();
        $commonSettings = $business ? $business->common_settings : [];
        $customerAppDomain = rtrim($commonSettings['customer_app_domain'] ?? '', '/');

        $jobOrderUrl = $customerAppDomain . '/check/phone/' . $job_order_id;
        $url = $welcome . ' ' . $jobOrderUrl;

        $smsResult = SmsUtil::sendEpusheg($info->mobile, $url);
        $smsSent = is_array($smsResult) ? $smsResult['success'] : $smsResult;

        // Log SMS with a reliable contact_id from contacts table
        SmsLog::create([
            'contact_id' => $info->contact_id,
            'transaction_id' => null,
            'job_sheet_id' => $job_order_id,
            'mobile' => $info->mobile,
            'message_content' => $url,
    ...
    public function sendsurveyAsSms($visitUrl, $mobile, $first_name)
    {
        //return response()->json(['data' => $visitUrl]);
        // $info = DB::table('contacts')
        //     // ->join('bookings', 'bookings.id', '=', 'repair_job_sheets.booking_id')
        //     // ->join('contacts', 'bookings.contact_id', '=', 'contacts.id')
        //     ->where('id', $contact_id)
        //     ->first();
        $welcome = 'اهلا ا/' . $first_name . ' الرجاء ملئ هذا الاستطلاع لتقييم خدماتنا ';
        $url = $welcome . $visitUrl;
        $response = $this->sendSmsToUsers($mobile, $url);

        return $response;
    }

    /**
     * Save social login settings to database
     *
     * @param Request $request
     * @param int $business_id
     * @return void
     */
    private function saveSocialLoginSettings(Request $request, $business_id)
    {
        ...
        if ($request->hasFile('apple_service_file')) {
            ...
            $existingSettings = $this->getSocialLoginSettings($business_id);
        ...
        System::updateOrCreate(
            ['key' => 'social_login_settings_' . $business_id],
            ['value' => json_encode($social_login_settings)]
        );
        
        // Clear cache
        cache()->forget('social_login_settings_' . $business_id);
    }
     ...
     * @return void
     */
    private function saveNotificationSettings(Request $request, $business_id)
    {
        $notification_settings = $request->input('notification_settings', []);
        
        // Save to system table
        System::updateOrCreate(
            ['key' => 'notification_settings_' . $business_id],
            ['value' => json_encode($notification_settings)]
        );
        
        // Clear cache
        cache()->forget('notification_settings_' . $business_id);
    }
}
