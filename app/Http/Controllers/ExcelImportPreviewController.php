...
class ExcelImportPreviewController extends Controller
{
    /**
     * All Utils instance.
     */
    ...
    public function index()
    {
        ...
        if ($zip_loaded === false) {
            ...
            return view('excel_import_preview.index')
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if (! empty($sub_category_name)) {
                        if (empty($category)) { // Check if main category was defined for this row
                            $is_valid = false;
                            $error_msg = "Main category is required for sub-category '$sub_category_name' in row no. $row_no";
                            break;
                        }
                        // Use updateOrCreate to ensure parent_id is correctly set or updated
    ...
    /**
     * Simple upload page for minimal Excel import.
     */
    public function simple()
    {
        ...
        return view('import_products.simple');
    }

    /**
     * Store products via simplified Excel with headers from test.xlsx.
     * Columns: اسم الصنف, كود الصنف 1, CATEGORY, SUB-CATEGORY, CARBRAND, MODEL_YEAR_RANGE, brand, MANAGE STOCK (1=yes 0=No)
     */
    public function storeSimple(Request $request)
    {
        ...
        try {
            ...
            if (! $request->hasFile('products_excel')) {
                return back()->with([ 'status' => ['success' => 0, 'msg' => 'لم يتم رفع ملف إكسل.'] ]);
            }

            $file = $request->file('products_excel');
            $parsed = Excel::toArray([], $file);
            if (empty($parsed) || empty($parsed[0])) {
                return back()->with([ 'status' => ['success' => 0, 'msg' => 'الملف فارغ أو غير صالح.'] ]);
            }

            $rows = $parsed[0];
            if (count($rows) < 2) {
                return back()->with([ 'status' => ['success' => 0, 'msg' => 'لا توجد بيانات بعد صف العناوين.'] ]);
            }
            ...
            $headers = array_map(function($h){ return is_string($h) ? trim($h) : $h; }, $rows[0]);
            $idx = [
                'name' => array_search('اسم الصنف', $headers),
                'sku' => array_search('كود الصنف 1', $headers),
                'category' => array_search('CATEGORY', $headers),
                'sub_category' => array_search('SUB-CATEGORY', $headers),
                'car_brand' => array_search('CARBRAND', $headers),
                'model_year_range' => array_search('MODEL_YEAR_RANGE', $headers),
                'brand' => array_search('brand', $headers),
                'manage_stock' => array_search('MANAGE STOCK (1=yes 0=No)', $headers),
            ];

            // Defaults & helpers
            $default_unit = Unit::where('business_id', $business_id)
                ->where('actual_name', 'قطعة')
                ->first();
            if (! $default_unit) {
            ...
            foreach (array_slice($rows, 1) as $i => $row) {
                $row_no = $i + 2; // considering header row

                $name = $idx['name'] !== false ? trim((string)($row[$idx['name']] ?? '')) : '';
                if ($name === '') {
                    // Skip empty-name rows
                    continue;
                }

                $sku = $idx['sku'] !== false ? trim((string)($row[$idx['sku']] ?? '')) : '';
                $category_name = $idx['category'] !== false ? trim((string)($row[$idx['category']] ?? '')) : '';
                $sub_category_name = $idx['sub_category'] !== false ? trim((string)($row[$idx['sub_category']] ?? '')) : '';
                $vehicle_brand_name = $idx['car_brand'] !== false ? trim((string)($row[$idx['car_brand']] ?? '')) : '';
                $myr = $idx['model_year_range'] !== false ? trim((string)($row[$idx['model_year_range']] ?? '')) : '';
                $product_brand_name = $idx['brand'] !== false ? trim((string)($row[$idx['brand']] ?? '')) : '';
                $manage_stock_val = $idx['manage_stock'] !== false ? trim((string)($row[$idx['manage_stock']] ?? '')) : '';
                $enable_stock = ($manage_stock_val === '1') ? 1 : 0;
                ...
                if ($sub_category_name !== '') {
                    if (! $category) {
                        $errors[] = "Main category missing for sub-category '$sub_category_name' at row $row_no";
                        continue;
                    }
                    $sub_category = Category::updateOrCreate(
                        ['business_id' => $business_id, 'name' => $sub_category_name, 'category_type' => 'product'],
                ...
                if ($product_brand_name !== '') {
                    $product_brand = Brands::firstOrCreate(
                        ['business_id' => $business_id, 'name' => $product_brand_name],
                        ['created_by' => $user_id]
                    );
                }
                ...
                if ($category) { $product_data['category_id'] = $category->id; }
                if ($sub_category) { $product_data['sub_category_id'] = $sub_category->id; }
                if ($product_brand) { $product_data['brand_id'] = $product_brand->id; }

                // Ensure unique SKU within business
                if ($sku !== '') {
                    $exists = Product::where('business_id', $business_id)->where('sku', $sku)->exists();
                    if ($exists) {
                        $errors[] = "SKU '$sku' already exists at row $row_no";
                        continue;
                    }
                }
                ...
                // Create a single variation with default zeros
                $this->productUtil->createSingleProductVariation(
                    $product->id,
                    $product->sku,
                    0, // item_level_purchase_price_total
                    0, // purchase_price_inc_tax
                    0, // profit_percent
                    0, // selling_price
                ...
                // Vehicle brand to bind compatibility
                $vehicle_brand = null;
                ...
                // Parse MODEL_YEAR_RANGE and save compatibility
                if ($vehicle_brand && $myr !== '') {
                    $segments = array_filter(array_map('trim', preg_split('/[;،]+/u', $myr)));
                    foreach ($segments as $seg) {
                        // Remove leading brand name if present
                        $segClean = $seg;
                        $brandPrefix = $vehicle_brand->name;
                        ...
                        // Extract years
                        $from = null; $to = null;
                        if (preg_match('/(\d{4})\s*[\x{2013}-]\s*(\d{4})/u', $segClean, $m)) { // en dash or hyphen
                            $from = intval($m[1]);
                            $to = intval($m[2]);
                            // Remove the year range part from model string
                            $model_str = trim(str_replace($m[0], '', $segClean));
                        } else {
                            // If single year appears
                            if (preg_match('/(\d{4})/u', $segClean, $m2)) {
                                $from = intval($m2[1]);
                                $to = $from;
                                $model_str = trim(str_replace($m2[1], '', $segClean));
                            } else {
                                $model_str = $segClean; // no year found; still record with null years
                            }
                        }

                        if ($model_str === '') { continue; }

                        // Find or create DeviceModel under vehicle brand
                        ...
                        ProductCompatibility::create([
                            ...
                            'brand_category_id' => $vehicle_brand->id,
            ...
            $msg = "تم إنشاء $created_count منتج/منتجات";
            if (! empty($errors)) {
                $msg .= '، مع بعض التحذيرات: ' . implode(' | ', $errors);
            }
            return back()->with([ 'status' => ['success' => 1, 'msg' => $msg] ]);

        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Simple import error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return back()->with([ 'status' => ['success' => 0, 'msg' => 'حدث خطأ أثناء الاستيراد المبسّط.'] ]);
        }
    }

    /**
     * NEW METHOD: Store products via the Warehouse Excel format provided in prompt.
     * Columns:
     * Index 7: الكود (SKU)
     * Index 8: اسم المنتج (Name)
     * Index 10: العدد (Quantity)
     * Index 11: سعر التكلفه (Cost Price)
     * Index 12: سعر البيع (Sale Price)
     * Index 13: المخزن (Warehouse Name)
     *
     * Logic: Exclude rows where Sale Price is #N/A or empty (these are categories).
     */
    public function storeWarehouseFormat(Request $request)
    {
        if (! auth()->user()->can('product.create')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        try {
            ...
            // Slice rows to skip the header row (index 0)
            foreach (array_slice($rows, 1) as $i => $row) {
                $row_no = $i + 2;

                // Map columns based on the file structure analysis
                $sku = isset($row[7]) ? trim((string)$row[7]) : '';
                $name = isset($row[8]) ? trim((string)$row[8]) : '';
                $quantity_raw = isset($row[10]) ? trim((string)$row[10]) : '0';
                $cost_price_raw = isset($row[11]) ? trim((string)$row[11]) : '0';
                $sell_price_raw = isset($row[12]) ? trim((string)$row[12]) : '';
                $warehouse_name = isset($row[13]) ? trim((string)$row[13]) : '';

                // EXCLUDE Logic: 
                // 1. If SKU is empty, *, or 0
                // 2. If Name is empty
                // 3. If Sell Price is #N/A or empty (This identifies Categories vs Products)
                
                if (empty($sku) || in_array($sku, ['*', '0']) || empty($name)) {
                    $skipped_count++;
                    continue;
                }

                if (empty($sell_price_raw) || strtoupper($sell_price_raw) == '#N/A') {
                    $skipped_count++;
                    continue;
                }

                // Clean numeric data (remove commas, currency symbols)
                $sell_price = (float) str_replace([',', ' '], '', $sell_price_raw);
                $cost_price = (float) str_replace([',', ' '], '', $cost_price_raw);
                $quantity = (float) str_replace([',', ' '], '', $quantity_raw);

                // Find Location by Warehouse Name
                $location = null;
                ...
                // Fallback to first location if specific warehouse not found
                if (! $location) {
                    $location = BusinessLocation::where('business_id', $business_id)->first();
                }

                if (! $location) {
                    $errors[] = "No location found for warehouse '$warehouse_name' at row $row_no";
                    continue;
                }
                ...
                $product_data = [
                    ...
                    'name' => $name,
                    'type' => 'single',
                    'enable_stock' => 1, // Enable stock since we are importing quantities
                    'unit_id' => $default_unit->id,
                    'sku' => $sku,
                    'barcode_type' => 'C128',
                ];
                ...
                // Calculate Variation Prices (Cost vs Sell)
                // Since we don't have tax info in the file, we assume 0 tax or use default
                $tax_amount = 0;
                $tax_type = 'exclusive';
                
                // Use existing utility to calculate properly
                $prices = $this->calculateVariationPrices($cost_price, $cost_price, $sell_price, $tax_amount, $tax_type, $default_profit_percent);
                ...
                // Add Opening Stock if Quantity > 0
                if ($quantity > 0) {
                    $opening_stock = [
                        'quantity' => $quantity,
            ...
            $msg = "تم استيراد $created_count منتج بنجاح.";
            if ($skipped_count > 0) {
                $msg .= " تم تجاهل $skipped_count صف (فئات/رؤوس).";
            }
            if (! empty($errors)) {
            ...
            DB::rollBack();
            \Log::error('Warehouse import error: ' . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return back()->with([ 'status' => ['success' => 0, 'msg' => 'حدث خطأ أثناء الاستيراد: ' . $e->getMessage()] ]);
        }
    }
    ...
    public function preview(Request $request)
    {
        ...
        try {
            ...
            $rows = $parsed[0];
            if (count($rows) < 2) {
                return back()->with('error', 'No data found after header row.');
            }
            ...
            $preview_rows = array_slice($rows, 1, $max_preview_rows);
            ...
            foreach ($headers as $index => $header) {
                $header_lower = strtolower(trim((string) $header));

                if (strpos($header_lower, 'اسم') !== false || strpos($header_lower, 'name') !== false) {
                    $idx_name = $index;
                }

                if (strpos($header_lower, 'كود') !== false || strpos($header_lower, 'sku') !== false || strpos($header_lower, 'code') !== false) {
                    $idx_sku = $index;
                }

                if (strpos($header_lower, 'category') !== false || strpos($header_lower, 'فئة') !== false) {
                    $idx_cat = $index;
                }

                if (strpos($header_lower, 'sub') !== false || strpos($header_lower, 'فرعية') !== false) {
                    $idx_sub = $index;
                }

                if (strpos($header_lower, 'brand') !== false || strpos($header_lower, 'ماركة') !== false) {
                    $idx_brand = $index;
                }

                if (strpos($header_lower, 'qty') !== false || strpos($header_lower, 'كمية') !== false || strpos($header_lower, 'stock') !== false || strpos($header_lower, 'العدد') !== false) {
                    $idx_qty = $index;
                }

                if (strpos($header_lower, 'cost') !== false || strpos($header_lower, 'سعر التكلفه') !== false || strpos($header_lower, 'سعر التكلفة') !== false) {
                    $idx_cost = $index;
                }

                if (strpos($header_lower, 'price') !== false || strpos($header_lower, 'سعر بيع') !== false || strpos($header_lower, 'سعر البيع') !== false) {
                    $idx_price = $index;
                }

                if (strpos($header_lower, 'location') !== false || strpos($header_lower, 'موقع') !== false || strpos($header_lower, 'مخزن') !== false || strpos($header_lower, 'المخزن') !== false || strpos($header_lower, 'warehouse') !== false) {
                    $idx_loc = $index;
                }
            }

            $preview_data = compact(
                'headers',
                'preview_rows',
                'total_rows',
                'idx_name',
            ...
            session(['excel_preview_data' => $preview_data]);

            return view('excel_import_preview.preview', $preview_data);
    ...
    /**
     * Show preview page from session data (for page refreshes).
     *
     * @return \Illuminate\Http\Response
     */
    public function showPreview()
    {
        ...
        $preview_data = session('excel_preview_data');

        if (! $preview_data) {
            return redirect()->route('excel_import_preview.index')
    ...
    public function generateSql(Request $request)
    {
        ...
        // This would generate SQL statements based on the uploaded data
        // Implementation would depend on your specific requirements
        
        return response()->json([
            'success' => true,
            'message' => 'SQL generation functionality to be implemented'
        ]);
    }
}
