...
class PurchaseReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $purchase = Transaction::where('business_id', $business_id)
                        ->where('type', 'purchase')
                        ->with(['purchase_lines', 'purchase_lines.sub_unit', 'purchase_lines.product'])
            ...
            foreach ($purchase->purchase_lines as $purchase_line) {
                $requested_raw = $return_quantities[$purchase_line->id] ?? 0;
                $requested_qty = $this->productUtil->num_uf($requested_raw);

                $multiplier = 1;
                if (! empty($purchase_line->sub_unit) && ! empty($purchase_line->sub_unit->base_unit_multiplier)) {
                    $multiplier = $purchase_line->sub_unit->base_unit_multiplier;
                }

                $requested_qty = max(0, $requested_qty * $multiplier);
                $normalized_returns[$purchase_line->id] = $requested_qty;

                $max_returnable = $purchase_line->quantity - $purchase_line->quantity_sold - $purchase_line->quantity_adjusted;

                if ($requested_qty > ($max_returnable + 0.00001)) {
                    $product_name = optional($purchase_line->product)->name ?? ('ID: '.$purchase_line->product_id);
                    $requested_formatted = round($requested_qty, 2);
                    $available_formatted = round(max($max_returnable, 0), 2);

                    $validation_error = __("Cannot return :requested units for :product because only :available are available to return.", [
                        'requested' => $requested_formatted,
            ...
            if (! empty($validation_error)) {
                return redirect()->back()
                    ->with('status', ['success' => 0, 'msg' => $validation_error])
                    ->withInput();
            }
            ...
            foreach ($purchase->purchase_lines as $purchase_line) {
                ...
                $return_quantity = $normalized_returns[$purchase_line->id] ?? 0;
