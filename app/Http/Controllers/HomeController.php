...
class HomeController extends Controller
{
    ...
    public function __construct(
        ...
        $this->AdminlteCustomPresenter = $data ;
    }

    public function markSelectedNotifications(\Illuminate\Http\Request $request)
    {
        ...
        $user = auth()->user();
        $ids = collect($validated['notification_ids']);
        $updated = 0;
        if ($ids->isNotEmpty()) {
            $updated = \Illuminate\Notifications\DatabaseNotification::whereIn('id', $ids)
                ->whereNull('read_at')
    ...
    public function index()
    {
        ...
        return view('home.home_page', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin'));
    }
    ...
    public function dashboardHome($id)
    {
        ...
        if (! auth()->user()->can('dashboard.data')) {
            return view('home.dashboard_item', ['id' => $id]);
        }
        ...
        $this->AdminlteCustomPresenter->process($id);

        if($id == "المنتجات"){
          $id =  "Products";
        }

        // Redirect to Treasury module if the ID is Treasury
        if($id == "Treasury" || $id == " Treasury " || $id == "الخزينة" || $id == " الخزينة"){
            return redirect('/treasury');
        }

        return view('home.dashboard_item', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin'),)->with('id', $id);
    }
    /**
    ...
    {
        // Load next page of UNREAD notifications only
        $notifications = auth()->user()->unreadNotifications()->orderBy('created_at', 'DESC')->paginate(10);

        $notifications_collection = $notifications->getCollection();
        $notifications_data = $this->commonUtil->parseNotifications($notifications_collection);

        // Do not mark as read on load; explicit actions will handle it
    ...
    public function getTotalUnreadNotifications()
    {
        ...
        foreach ($unread_notifications as $unread_notification) {
            if (isset($unread_notification->data['show_popup'])) {
