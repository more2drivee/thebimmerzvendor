...

{
        ...
        if ($request->ajax()) {
            ...
            $product_details = $query->select(
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='sell', TSL.quantity - TSL.quantity_returned, -1* TPL.quantity) ) FROM transactions
                        ...
                        WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell' $location_filter
                ...
                DB::raw("(SELECT SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ) FROM transactions
                        ...
                        WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell_transfer' $location_filter
            ...
            return view('report.partials.stock_details_table')
     ...
     * Comprehensive sales report: product sales details and supplier summary
    ...
    public function getComprehensiveSalesReport()

    {
        ...
        if (request()->ajax()) {
            ...
            $baseQuery = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines as SL', 'SL.id', '=', 'transaction_sell_lines_purchase_lines.sell_line_id')

                ->leftJoin('transactions as sale', 'SL.transaction_id', '=', 'sale.id')

                ->leftJoin('purchase_lines as PL', 'PL.id', '=', 'transaction_sell_lines_purchase_lines.purchase_line_id')

                ->leftJoin('transactions as purchase', 'PL.transaction_id', '=', 'purchase.id')
                ...
                ->whereIn('sale.type', ['sell', 'under processing']);
            ...
            //     $baseQuery->whereIn('sale.location_id', $permitted_locations);
            ...
            if (! empty(request()->start_date) && ! empty(request()->end_date)) {
                ...
                $baseQuery->whereDate('sale.transaction_date', '>=', $start)
            ...
            if (! empty(request()->sale_payment_status)) {

                $baseQuery->where('sale.payment_status', request()->sale_payment_status);

            }



            // Purchase payment status filter

            if (! empty(request()->purchase_payment_status)) {

                $baseQuery->where('purchase.payment_status', request()->purchase_payment_status);
            ...
            if (! empty(request()->supplier_id)) {
                ...
                $baseQuery->where('suppliers.id', request()->supplier_id);
            ...
            if (! empty(request()->location_id)) {
                ...
                $baseQuery->where('sale.location_id', request()->location_id);

            }


            $detailsQuery = (clone $baseQuery)
                ...
                ->select(
                    ...
                    DB::raw('SUM(transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) as sold_qty'),

                    DB::raw('IFNULL(SUM((transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned) * PL.purchase_price_inc_tax) / NULLIF(SUM((transaction_sell_lines_purchase_lines.quantity - transaction_sell_lines_purchase_lines.qty_returned)), 0), 0) as purchase_unit_price'),

                    DB::raw('GROUP_CONCAT(DISTINCT suppliers.name ORDER BY suppliers.name SEPARATOR ", ") as supplier_name'),

                    DB::raw('GROUP_CONCAT(DISTINCT suppliers.supplier_business_name ORDER BY suppliers.supplier_business_name SEPARATOR ", ") as supplier_business_name'),

                    DB::raw('GROUP_CONCAT(DISTINCT suppliers.mobile ORDER BY suppliers.mobile SEPARATOR ", ") as supplier_mobile'),

                    DB::raw('GROUP_CONCAT(DISTINCT CONCAT(purchase.id, ":", purchase.ref_no, ":", COALESCE(purchase.payment_status, "")) ORDER BY purchase.id SEPARATOR ",") as purchase_pairs'),

                    DB::raw('GROUP_CONCAT(DISTINCT COALESCE(purchase.payment_status, "") ORDER BY purchase.payment_status SEPARATOR ",") as purchase_payment_statuses')
            ...
            $formatPaymentStatus = function (?string $status) {
            ...
            return Datatables::of($detailsQuery)
                ...
                ->editColumn('sold_qty', function ($row) {
                    ...
                    return '<span data-is_quantity="true" class="display_currency" data-currency_symbol=false data-orig-value="' . htmlspecialchars($qty, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($qty, ENT_QUOTES, 'UTF-8') . '</span> ' . htmlspecialchars($unit, ENT_QUOTES, 'UTF-8');
                ...
                ->editColumn('selling_price', function ($row) {
                    ...
                    return '<span class="display_currency" data-currency_symbol=true data-orig-value="' . htmlspecialchars($price, ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($price, ENT_QUOTES, 'UTF-8') . '</span>';

                })

                ->editColumn('purchase_unit_price', function ($row) {

                    $price = (float) ($row->purchase_unit_price ?? 0);
                ...
                ->addColumn('transaction_number', function ($row) {
                    ...
                    return '<button type="button" class="btn btn-link btn-modal" data-href="' . action([\App\Http\Controllers\SellController::class, 'show'], [$row->sale_id]) . '" data-container=".view_modal">' . htmlspecialchars($row->invoice_no, ENT_QUOTES, 'UTF-8') . '</button>';

                })

                ->editColumn('sale_payment_status', function ($row) use ($formatPaymentStatus) {

                    return $formatPaymentStatus($row->sale_payment_status ?? '');

                })

                ->addColumn('purchase_ref', function ($row) use ($formatPaymentStatus) {

                    $html = '';

                    if (! empty($row->purchase_pairs)) {

                        $pairs = explode(',', $row->purchase_pairs);
                        ...
                        foreach ($pairs as $pair) {
                            ...
                            if (count($parts) >= 2) {
                                ...
                                if (! empty($ref)) {
                                    ...
                                    $label = htmlspecialchars($ref, ENT_QUOTES, 'UTF-8');



                                    $html .= '<button type="button" class="btn btn-link btn-modal" data-href="' . action([\App\Http\Controllers\PurchaseController::class, 'show'], [$pid]) . '" data-container=".view_modal">' . $label . '</button>';
                ...
                ->editColumn('purchase_payment_statuses', function ($row) use ($formatPaymentStatus) {

                    if (empty($row->purchase_payment_statuses)) {
                    ...
                    $statuses = array_unique(array_filter(explode(',', $row->purchase_payment_statuses)));
                    ...
                    $labels = array_map(function ($status) use ($formatPaymentStatus) {
                ...
                ->editColumn('supplier_name', function($row) {
                    ...
                    if (!empty($row->supplier_business_name)) {
                        ...
                        $output .= htmlspecialchars($row->supplier_business_name, ENT_QUOTES, 'UTF-8') . ',<br>';

                    }

                    if (!empty($row->supplier_name)) {

                        $output .= htmlspecialchars($row->supplier_name, ENT_QUOTES, 'UTF-8');
                ...
                ->rawColumns(['sold_qty', 'selling_price', 'purchase_unit_price', 'transaction_number', 'sale_payment_status', 'purchase_ref', 'purchase_payment_statuses', 'supplier_name'])
        ...
        return view('report.comprehensive_sales_report')->with(compact('suppliers', 'locations'));
