...
public function MarkAllReadNotification(Request $request)
{
    ...
    if (!is_array($notificationIDs) || empty($notificationIDs)) {
        return response()->json([
            'success' => false,
            'message' => 'Invalid or empty notification IDs',
        ], 400);
    }
   ...
   public function GetNotifications(Request $request)
{
    ...
    foreach ($notifications as $notification) {
        ...
        $title = $payload['booking_name'] ?? ($payload['message'] ?? 'Notification');
        $body = '';
        $contactName = $payload['contact_name'] ?? '';
        $deviceName = $payload['device_name'] ?? '';
        $bookingStart = $payload['booking_start'] ?? '';
        $locationName = $payload['location_name'] ?? '';
        $note = $payload['note'] ?? '';
        $service = $payload['service'] ?? '';

        if ($type === 'App\\Notifications\\BookingNotification') {
            $body = "Customer: $contactName\nDevice: $deviceName\nBooking: $bookingStart\nLocation: $locationName\nService: $service\nNote: $note";
        } else {
            $body = $payload['body'] ?? '';
        }
        ...
        public function getByStatus(Request $request)
{
   ...
   $notifications = DB::table('notifications')
    ...
    ->when($status !== 'all', function ($q) use ($status) {
        $q->where('notification_status', $status);
    })
    ->latest()
    ->paginate(10);
    ...
    private function handleBookingAction(Request $request)
{
    ...
    $booking = DB::table('bookings')->where('id', $bookingId)->where('booking_status', 'request')->first();
    if (!$booking) {
        return response()->json([
            'success' => false,
   ...
   DB::table('notifications')
    ->whereRaw(
        "JSON_EXTRACT(data, '$.booking_id') = CAST(? AS JSON)",
        [$bookingId]
    )
    ->update([
        'status'              => 1,
        'notification_status' => $status,
        'actionBy' => $status . ' by ' . $user->first_name . ' ' . $user->last_name,
        'updated_at'          => now()
    ]);
...
public function NotificationAction(Request $request)
{
    ...
    if (!$type || !$action) {
        return response()->json([
            'success' => false,
            'message' => 'Missing type or action'
        ], 400);
    }

    return match ($type) {
        'booking'  => $this->handleBookingAction($request),
        // 'order' => $this->handleOrderAction($request),
