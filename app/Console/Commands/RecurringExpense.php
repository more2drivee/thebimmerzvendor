...
class RecurringExpense extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            foreach ($transactions as $transaction) {
                ...
                try {
                    ...
                    if ($created_by) {
                        ...
                        $this->warn('Original user (ID: ' . $transaction->created_by . ') not found, skipping notification');
                    }

                    //if admin is different
                    if ($created_by && $created_by->id != $transaction->business->owner_id) {
                        $admin = User::find($transaction->business->owner_id);
                        if ($admin) {
                            $this->notificationUtil->recurringExpenseNotification($admin, $recurring_expense);
                        } else {
                            $this->warn('Admin user (ID: ' . $transaction->business->owner_id . ') not found, skipping admin notification');
                        }
                    }
                    DB::commit();
                } catch (\Exception $e) {
            ...
            $this->error('RecurringExpense command failed: ' . $e->getMessage());
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
            exit($e->getMessage());
        }
    }
