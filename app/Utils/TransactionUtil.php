...
use App\Exceptions\PaymentAccountInsufficientBalance;
...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'type' => $sale_type,
            'status' => $input['status'],
            'repair_job_sheet_id' => !empty($input['repair_job_sheet_id']) ? $input['repair_job_sheet_id'] : null,
            'repair_status_id' => !empty($input['repair_status_id']) ? $input['repair_status_id'] : null,
            'repair_brand_id' => !empty($input['repair_brand_id']) ? $input['repair_brand_id'] : null,
            'repair_model_id' => !empty($input['repair_model_id']) ? $input['repair_model_id'] : null,
            ...
            'repair_device_id' => ! empty($input['repair_device_id']) ? $input['repair_device_id'] : null,
            'repair_device_km' => ! empty($input['repair_device_km']) ? $input['repair_device_km'] : null,
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
{
    ...
    // Check if $invoice_total is false and set defaults if so
    if ($invoice_total === false || !is_array($invoice_total)) {
        $invoice_total = [
            'total_before_tax' => 0,
            'tax' => 0,
        ];
    ...
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                // Make sure the referenced sell line actually exists; if not, fall through and
                // treat this product as a new line instead of trying to edit a null model.
                $existing_sell_line = TransactionSellLine::find($product['transaction_sell_lines_id']);

                if (! empty($existing_sell_line)) {
                    ...
                    // We have successfully handled this product as an edit, move to next one.
                    continue;
                }
                // If we reach here, the provided transaction_sell_lines_id was invalid; treat
                // this as a new line below instead of editing a non-existent line.
            }

            // New line creation (no valid transaction_sell_lines_id)
    ...
    private function adjustQuantity($location_id, $product_id, $variation_id, $increment_qty)
    {
        if ($increment_qty != 0) {
            ...
            if ($enable_stock == 1) {
                app(\App\Utils\ProductUtil::class)->safeAdjustQtyAvailable(
                    $product_id,
                    $variation_id,
                    $location_id,
                    $increment_qty
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        foreach ($payments as $payment) {
            ...
            if (! empty($payment['payment_id'])) {
                ...
                //Check payment method balance for non-advance payments

                if ($payment['method'] != 'advance' && in_array($transaction->type, ['purchase', 'purchase_return', 'expense', 'expense_refund'])) {

                    $treasury_repo = new TreasuryRepository();
                    $payment_methods = $this->payment_types();
                    $balances = $treasury_repo->getPaymentMethodBalances($transaction->business_id, $payment_methods);

                    $payment_method_balance = 0;
                    foreach ($balances as $balance) {
                        if ($balance['id'] === $payment['method']) {
                            $payment_method_balance = $balance['balance'];
                            break;
                        }
                    }

                    if ($payment_amount > $payment_method_balance) {
                        throw new PaymentAccountInsufficientBalance(__('lang_v1.required_payment_account_balance_not_available'));
                    }
                }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_mobile_number == 1 && ! empty($location_details->mobile)) {
            $output['contact'] .=  $location_details->mobile;
        }
        ...
        $output['payment_status_label'] = 'الحالة:';
        $paymentStatusKey = $transaction->payment_status;
        $output['payment_status'] = match ($paymentStatusKey) {
            'due' => 'مستحق',
            'paid' => 'مدفوع',
            'partial' => 'جزئي',
            default => '-',
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            $sell_line_relations = ['modifiers', 'sub_unit', 'warranties', 'product.brand', 'variations', 'variations.product_variation', 'product.unit', 'product.category'];
        ...
        // Calculate subtotal: sum of all sell lines before discount
        $subtotal = 0;
        foreach ($transaction->sell_lines as $line) {
            $quantity = $line->quantity ?? 1;
            $unit_price = $line->unit_price_before_discount ?? $line->unit_price ?? 0;
        ...
        if ($transaction->discount_type == 'percentage') {
            $transaction_discount = ($transaction->discount_amount / 100) * $subtotal;
        } else {
            $transaction_discount = $transaction->discount_amount;
        }
        ...
        $total_discount = $transaction_discount + $line_discount;
        
        $output['subtotal'] = ($subtotal != 0) ? $this->num_f($subtotal, $show_currency, $business_details) : 0;
        $output['subtotal_unformatted'] = ($subtotal != 0) ? $subtotal : 0;
        ...
        $transaction_discount = 0;
        if ($transaction->discount_type == 'percentage') {
            $transaction_discount = ($transaction->discount_amount / 100) * $transaction->total_before_tax;
        } else {
            $transaction_discount = $transaction->discount_amount;
        }
        ...
        $output['total_line_discount'] = ($line_discount != 0) ? $this->num_f($line_discount, $show_currency, $business_details) : 0;
        ...
        if ($transaction_type == 'sell') {
            ...
            if ($transaction->status == 'final' && $il->show_previous_bal == 1) {
            ...
            // Primary payment method (for headers like "طريقة الدفع"), derived from payment_lines, independent of show_payments
            $output['primary_payment_method'] = null;
            $transaction_payment_lines = $transaction->payment_lines;
            if (! $transaction_payment_lines->isEmpty()) {
                $payment_types = $this->payment_types($transaction->location_id, true);
                foreach ($transaction_payment_lines as $payment_line) {
                    // Skip pure change-return / negative lines
                    if (! empty($payment_line->is_return)) {
                        continue;
                    }

                    $method_key = $payment_line->method;
                    $display_method = ! empty($payment_types[$method_key]) ? $payment_types[$method_key] : $method_key;

                    $output['primary_payment_method'] = $display_method;
                    break;
                }
            }

            // Detailed payment breakdown (used by layouts that show payments table)
            $output['payments'] = [];
            if ($il->show_payments == 1 && ! $transaction_payment_lines->isEmpty()) {
                // Reuse the same payment types map if already built above
                if (! isset($payment_types)) {
                    $payment_types = $this->payment_types($transaction->location_id, true);
                }

                $payments = $transaction_payment_lines->toArray();
                if (! empty($payments)) {
        ...
        // Add job sheet data if transaction has a repair_job_sheet_id
        if (!empty($transaction->repair_job_sheet_id)) {
            // Get the job sheet using joins and select
            $job_sheet = DB::table('repair_job_sheets')
                ...
                ->leftJoin('users', 'repair_job_sheets.created_by', '=', 'users.id')
                 ->leftJoin('maintenance_note', function($join) {
                    $join->on('repair_job_sheets.id', '=', 'maintenance_note.job_sheet_id')
                         ->where('maintenance_note.category_status', '=', 'note')
                         ->limit(1);
                })
                ->where('repair_job_sheets.id', $transaction->repair_job_sheet_id)
                ->select([
                    'repair_job_sheets.*',
                    'bookings.booking_start AS entry_date',
                ...
                ->first();
                   ...
                   if (!empty($job_sheet)) {
                // Fill the output with job sheet details
                $output['service_no'] = $job_sheet->job_sheet_no ?? '';
                $output['license_plate'] = $job_sheet->plate_number ?? '';
                $output['vin_number'] = $job_sheet->chassis_number ?? '';
                $output['repair_serial_no'] = $job_sheet->km ?? '';
                $output['car_brand'] = $job_sheet->device ?? '';
                $output['car_model'] = $job_sheet->device_model ?? '';
                $output['car_color'] = $job_sheet->color ?? '';
                $output['chassis_number'] = $job_sheet->chassis_number ?? '';
                $output['plate_number'] = $job_sheet->plate_number ?? '';
                $output['odometer'] = $job_sheet->km ?? '';
                $output['manufacturing_year'] = $job_sheet->manufacturing_year ?? '';
                $output['repair_status'] = $job_sheet->status ?? '';
                $output['job_sheet_no'] = $job_sheet->job_sheet_no ?? '';
                $output['job_sheet_date'] = $job_sheet->created_at ?? ''; // Ensure created_at is selected or set default
                $output['service_staff'] = $job_sheet->added_by ?? '';
                $output['content'] = $job_sheet->content ?? '';
               

                $output['show_jobsheet_details'] = true;
            }
        }else{
             
                   if (!empty($transaction->repair_device_id)) {
                    $contact_device = DB::table('contact_device')->where('id', $transaction->repair_device_id)->first();
                    if (!empty($contact_device)) {
                        // Use contact_device data as fallback for missing job sheet data
                        $output['license_plate'] =  ($contact_device->plate_number ?? '');
                        $output['plate_number'] =  ($contact_device->plate_number ?? '');
                        $output['vin_number'] =  ($contact_device->chassis_number ?? '');
                        $output['chassis_number'] =  ($contact_device->chassis_number ?? '');
                        $output['car_color'] = ($contact_device->color ?? '');
                        $output['manufacturing_year'] = ($contact_device->manufacturing_year ?? '');
                        $output['car_brand'] =  (DB::table('categories')->where('id', $contact_device->device_id)->value('name') ?? '');
                        $output['car_model'] =  (DB::table('repair_device_models')->where('id', $contact_device->models_id)->value('name') ?? '');
                    }
                }

                // Use transaction repair_device_km as fallback for km/odometer data
                if ( !empty($transaction->repair_device_km)) {
                    $output['repair_serial_no'] = $transaction->repair_device_km;
                    $output['odometer'] = $transaction->repair_device_km;
                }
        }
        ...
        // Expose invoice layout common settings (used by some receipt views)
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'category_name' => ! empty($cat) ? $cat->name : '',
                'enable_stock' => $product->enable_stock,
                'virtual_product' => $product->virtual_product,
                'is_client_flagged' => $product->is_client_flagged,
                ...
                'unit_price_before_discount_inc_tax' => $this->num_f($line->unit_price_before_discount + $line->item_tax, false, $business_details),
                'unit_price_before_discount_inc_tax_uf' => $line->unit_price_before_discount + $line->item_tax,
                ...
                'line_total_before_discount' => $this->num_f(($line->unit_price_before_discount + $line->item_tax) * $line->quantity, false, $business_details),
                'line_total_before_discount_uf' => ($line->unit_price_before_discount + $line->item_tax) * $line->quantity,
            ...
            // Always include brand_name for use in receipt templates
            $line_array['brand_name'] = ! empty($brand->name) ? $brand->name : '';
            if ($il->show_sku == 1) {
    ...
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null)
    {
        if ($status == 'final') {
            ...
            return $invoice_no;
        } elseif ($status == 'under processing' || $status == '') {
            $ref_count = $this->setAndGetReferenceCount('under processing', $business_id);
            $invoice_no = $this->generateReferenceNumber('under processing', $ref_count, $business_id);
    ...
    public function adjustMappingPurchaseSell(
        ...
        } elseif ($status_before == 'draft' && ($transaction->status == 'final' || $transaction->status == 'under processing')) {
            // When moving from draft to final or under processing, allocate purchases for all sell lines
            $this->mapPurchaseSell($business, $transaction->sell_lines, 'purchase');
        } elseif (($status_before == 'final' || $status_before == 'under processing') && ($transaction->status == 'final' || $transaction->status == 'under processing')) {
            // When transaction is final or under processing, treat as final for purchase mapping
            //Handle deleted line
            if (! empty($deleted_line_ids)) {
                $deleted_sell_purchases = TransactionSellLinesPurchaseLines::whereIn('sell_line_id', $deleted_line_ids)
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (! empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                // If this sale is linked to a repair job sheet, also reset
                // any product_joborder flags for that job order so that
                // nothing remains in an approved/delivered state after
                // the invoice has been cancelled.
                if (! empty($transaction->repair_job_sheet_id)) {
                    try {
                        DB::table('product_joborder')
                            ->where('job_order_id', (int) $transaction->repair_job_sheet_id)
                            ->update([
                                'client_approval'   => 0,
                                'delivered_status'  => 0,
                                'out_for_deliver'   => 0,
                        ...
                        // Do not block sale deletion if this ancillary
                        // sync fails; just log for later investigation.
                        \Log::warning('Failed to reset product_joborder flags on sale delete', [
                            'transaction_id' => $transaction->id,
                            'job_sheet_id'   => $transaction->repair_job_sheet_id,
                            'error'          => $e->getMessage(),
                        ]);
                    }
                }
    ...
    {
        $purchases = Transaction::with(['contact', 'location', 'payment_lines', 'tax'])
            ->leftJoin('repair_job_sheets as js', 'transactions.repair_job_sheet_id', '=', 'js.id')
            ...
            ->select([
                ...
                DB::raw('COALESCE(js.job_sheet_no, "") as job_sheet_no'),
                DB::raw('(SELECT COALESCE(SUM(TP.amount),0) FROM transaction_payments AS TP WHERE TP.transaction_id = transactions.id) as amount_paid'),
                DB::raw('(SELECT PR.id FROM transactions PR WHERE PR.return_parent_id = transactions.id LIMIT 1) as return_transaction_id'),
                DB::raw('(SELECT COALESCE(SUM(TP2.amount),0) FROM transaction_payments AS TP2 WHERE TP2.transaction_id = (SELECT PR2.id FROM transactions PR2 WHERE PR2.return_parent_id = transactions.id LIMIT 1)) as return_paid'),
                DB::raw('(SELECT COUNT(*) FROM transactions PRc WHERE PRc.return_parent_id = transactions.id) as return_exists'),
                DB::raw('COALESCE((SELECT PR3.final_total FROM transactions PR3 WHERE PR3.return_parent_id = transactions.id LIMIT 1), 0) as amount_return'),
                DB::raw('CONCAT(COALESCE(u.surname, "")," ",COALESCE(u.first_name, "")," ",COALESCE(u.last_name,"")) as added_by'),
                DB::raw('(SELECT COALESCE(SUM(PL.asked_qty), 0) FROM purchase_lines AS PL WHERE PL.transaction_id = transactions.id) as total_asked_qty'),
                DB::raw('(SELECT COALESCE(SUM(PL.quantity), 0) FROM purchase_lines AS PL WHERE PL.transaction_id = transactions.id) as total_received_qty')
            ]);
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        $transaction_data = $request->only(['ref_no', 'transaction_date',
            ...
            'expense_category_id', 'tax_id', 'contact_id', 'invoice_ref']);
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        if ($request->has('invoice_ref')) {
            $transaction_data['invoice_ref'] = $request->input('invoice_ref');
        }
    ...
    /**
     * Get payment method balance for a specific location and business
     * Based on Treasury module's balance calculation
     *
     * @param int $business_id
     ...
     * @return float
     */
    public function getPaymentMethodBalance($business_id, $location_id, $payment_method)
    {
        // Income types: sell, opening_balance, sell_return, purchase_return
        $income_query = DB::table('transaction_payments')
            ...
            ->whereIn('transactions.type', ['sell', 'opening_balance', 'sell_return', 'purchase_return']);
        ...
        // Expense types: expense, purchase, payroll
        $expense_query = DB::table('transaction_payments')
            ->join('transactions', 'transaction_payments.transaction_id', '=', 'transactions.id')
            ->where('transactions.business_id', $business_id)
            ->where('transaction_payments.method', $payment_method)
            ->whereNotIn('transactions.status', ['draft', 'quotation', 'pending'])
            ->whereIn('transactions.type', ['expense', 'purchase', 'payroll']);
