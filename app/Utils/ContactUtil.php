...
class ContactUtil extends Util
{
    ...
    public function createNewContact($input, $device)
    {
        //Check Contact id
        $count = 0;
        ...
        if ($count == 0) {
            ...
            // Fallback: force contact_type to individual if not already set
            if (!isset($input['contact_type']) || empty($input['contact_type'])) {
                $input['contact_type'] = 'individual';
            }

            $contact = Contact::create($input);
            $contactdeviceid = $contact->id;
          
            $has_any_device_data = false;
            if (!empty($device)) {
                $device_keys_to_check = [
                    'gehad_category_id',
                    'gehad_model_id',
                    'manufacturing_year',
                    'color',
                ...
                foreach ($device_keys_to_check as $key) {
                    if (isset($device[$key]) && $device[$key] !== null && $device[$key] !== '') {
                        $has_any_device_data = true;
                        break;
                    }
                }
            }

            if (!empty($device) && isset($device['gehad_category_id'], $device['gehad_model_id'], $device['manufacturing_year'], $device['color'], $device['plate_number'])) {
                $modelBelongsToBrand = DB::table('repair_device_models')
                    ->where('id', $device['gehad_model_id'])
                    ->where('device_id', $device['gehad_category_id'])
                    ->exists();

                if (!$modelBelongsToBrand) {
                    Log::error('Device model does not belong to brand; skipping contact_device insert', [
                ...
                DB::table('contact_device')->insert([
                    'device_id' => $device['gehad_category_id'],
                    'models_id' => $device['gehad_model_id'],
                    'color' => $device['color'],
                    'chassis_number' => $device['chassis_number'],
                    'plate_number' => $device['plate_number'],
                    'manufacturing_year' => $device['manufacturing_year'],
                    'car_type' => $device['car_type'],
                    'contact_id' => $contact->id,
                ]);
                }
            } else {
                if ($has_any_device_data) {
                    Log::error('Device data is missing or incomplete:', ['device' => $device]);
                }
            }
            ...
            $_SESSION["contactid"] = $contactdeviceid;
