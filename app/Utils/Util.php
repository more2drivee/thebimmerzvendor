...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types['custom_pay_11'] = ! empty($custom_labels['payments']['custom_pay_11']) ? $custom_labels['payments']['custom_pay_11'] : __('lang_v1.custom_payment', ['number' => 11]);
        $payment_types['custom_pay_12'] = ! empty($custom_labels['payments']['custom_pay_12']) ? $custom_labels['payments']['custom_pay_12'] : __('lang_v1.custom_payment', ['number' => 12]);
        $payment_types['custom_pay_13'] = ! empty($custom_labels['payments']['custom_pay_13']) ? $custom_labels['payments']['custom_pay_13'] : __('lang_v1.custom_payment', ['number' => 13]);
        $payment_types['custom_pay_14'] = ! empty($custom_labels['payments']['custom_pay_14']) ? $custom_labels['payments']['custom_pay_14'] : __('lang_v1.custom_payment', ['number' => 14]);
        $payment_types['custom_pay_15'] = ! empty($custom_labels['payments']['custom_pay_15']) ? $custom_labels['payments']['custom_pay_15'] : __('lang_v1.custom_payment', ['number' => 15]);
        $payment_types['custom_pay_16'] = ! empty($custom_labels['payments']['custom_pay_16']) ? $custom_labels['payments']['custom_pay_16'] : __('lang_v1.custom_payment', ['number' => 16]);
        $payment_types['custom_pay_17'] = ! empty($custom_labels['payments']['custom_pay_17']) ? $custom_labels['payments']['custom_pay_17'] : __('lang_v1.custom_payment', ['number' => 17]);
    ...
    {
        $mysql_format = $time ? 'Y-m-d H:i:s' : 'Y-m-d';

        // Handle empty or literal "now"
        if (empty($date) || strtolower(trim($date)) === 'now') {
            return $time ? \Carbon::now()->format($mysql_format) : \Carbon::today()->format($mysql_format);
        }

        // Expected format based on business settings
        ...
        // Try a list of common alternative formats
        $fallback_formats = $time
            ? ['Y-m-d H:i:s', 'Y-m-d\TH:i:s', 'Y-m-d\TH:i', 'd/m/Y H:i', 'd-m-Y H:i', 'm/d/Y H:i', 'Y/m/d H:i']
        ...
        // Fallback to Carbon::parse which handles many free-form strings
        try {
            return \Carbon::parse($date)->format($mysql_format);
        } catch (\Throwable $th) {
            \Log::warning('uf_date failed to parse date, falling back to current date', ['value' => $date, 'time' => $time]);
            return $time ? \Carbon::now()->format($mysql_format) : \Carbon::today()->format($mysql_format);
        }
    }
    // public function uf_date($date, $time = false)
    ...
    public function getInvoiceUrl($transaction_id, $business_id)
    {
        ...
        // Prefer using invoice number in the URL if available
        // if (! empty($transaction->invoice_no)) {
        //     if ($transaction->is_quotation == 1) {
        //         return route('show_quote', ['token' => $transaction->invoice_no]);
        //     }

        //     return route('show_invoice', ['token' => $transaction->invoice_no]);
        // }

        // Fallback: keep existing token-based behaviour if no invoice number
    ...
    public function get_admins($business_id)
    {
        ...
        return $admins;
          ...
          public function get_adminsAndCashers($business_id)
{
        if (empty($business_id)) {
        ...
        // Get admins
        $admins = User::where('business_id', $business_id)
                      ->role('Admin#'.$business_id)
                      ->get();

        // Get users with specific permissions
        $users = User::where('business_id', $business_id)
                     ->where(function($query) use ($notification_permissions) {
                         // Direct permissions
                         $query->whereHas('permissions', function($q) use ($notification_permissions) {
                             $q->whereIn('name', $notification_permissions);
                         })
                         // Or permissions through roles
                         ->orWhereHas('roles', function($q) use ($notification_permissions) {
                             $q->whereHas('permissions', function($q2) use ($notification_permissions) {
                                 $q2->whereIn('name', $notification_permissions);
                             });
                         });
                     })
                     ->distinct()
        ...
        foreach ($users as $user) {
            Log::info('User notification permissions debug', [
                'user_id' => $user->id,
        ...
        $usersAndAdmins = $users->merge($admins)->unique('id');
    ...
    public function updateEditedSellLineCombo($lines, $location_id, $adjust_stock = true)
    {
        ...
        foreach ($lines as $key => $line) {
            ...
            if ($difference != 0) {
                ...
                if ($adjust_stock) {
                    // Use centralized safe adjust to avoid duplicates/races
    ...
    public function parseNotifications($notifications)
    {
        ...
        foreach ($notifications as $notification) {
                    ...
                    foreach ($module_notification_data as $module_data) {
                        if (! empty($module_data)) {
                            if (!isset($module_data['id'])) {
                                $module_data['id'] = $notification->id;
                            }
            ...
            if (! $notification_handled && ! empty($data)) {
                $notifications_data[] = [
                    'msg' => $data['msg'] ?? ($data['message'] ?? ''),
                    'icon_class' => $data['icon_class'] ?? 'fas fa-info-circle bg-aqua',
                    'link' => $data['link'] ?? '#',
                    'read_at' => $notification->read_at,
