...
class RestaurantUtil extends Util
{
    ...
    //     $query = Booking::where('business_id', $filters['business_id'])->get();
    ...
    public function getBookingsForCalendar($filters)
    {
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            $query->whereBetween('booking_start', [$filters['start_date'], $filters['end_date']]);
        }
        ...
        // Apply location filter if location_id is provided (must be within permitted locations)
        if (!empty($filters['location_id'])) {
            $query->where('location_id', $filters['location_id']);
        }

        // Eager load relationships to avoid N+1 queries
        $query->with(['customer', 'table']);

        // Fetch the bookings
        $bookings = $query->get();

        // Format the bookings into calendar events
        $events = [];
        ...
        foreach ($bookings as $booking) {
            ...
            // Set event colors based on booking status
            ...
            // Override colors if a custom color is provided in filters
            if (!empty($filters['color'])) {
                $backgroundColor = $filters['color'];
                $borderColor = $filters['color'];
            }
