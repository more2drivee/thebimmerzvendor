...
class ProductUtil extends Util
{
            ...
            if (! empty($value['variations'])) {
                ...
                foreach ($value['variations'] as $k => $v) {
                    ...
                    $variation_data[] = [
                        ...
                        'sell_price_inc_tax' => $sell_price_inc_tax ?? 0,
                    ];
                    $c++;
        ...
        foreach ($input_variations_edit as $key => $value) {
            ...
            if (! empty($value['variations_edit'])) {
                foreach ($value['variations_edit'] as $k => $v) {
                    ...
                    if (isset($v['default_sell_price']) && $v['default_sell_price'] !== '') {
                        $default_sell_price = $this->num_uf($v['default_sell_price']);
                    }

                    $sell_price_inc_tax = null;
                    if (isset($v['sell_price_inc_tax']) && $v['sell_price_inc_tax'] !== '') {
                        $sell_price_inc_tax = $this->num_uf($v['sell_price_inc_tax']);
                    }

                    if (is_null($default_sell_price) && ! is_null($sell_price_inc_tax)) {
                        $default_sell_price = $this->calc_percentage_base($sell_price_inc_tax, $tax_rate);
                    }

                    if (is_null($sell_price_inc_tax) && ! is_null($default_sell_price)) {
                        $sell_price_inc_tax = $this->calc_percentage($default_sell_price, $tax_rate, $default_sell_price);
                    }
    ...
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true)
    {
        ...
        if ($product->enable_stock == 1 && $qty_difference != 0) {
            ...
            $this->safeAdjustQtyAvailable($product_id, $variation_id, $location_id, $qty_difference, $variation ? $variation->product_variation_id : null);
        }
    ...
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0)
    {
        ...
        if ($product->enable_stock == 1) {
            ...
            $this->safeAdjustQtyAvailable($product_id, $variation_id, $location_id, -1 * $qty_difference, $variation ? $variation->product_variation_id : null);
        }
    ...
    /**
     * Safely adjust qty_available for a variation/location ensuring single row, avoiding races/duplicates.
     *
     * @param int $product_id
     * @param int $variation_id
     * @param int $location_id
     * @param float|int $delta positive to increment, negative to decrement
     * @param int|null $product_variation_id
     * @return float The updated qty_available
     */
    public function safeAdjustQtyAvailable($product_id, $variation_id, $location_id, $delta, $product_variation_id = null)
    {
        return DB::transaction(function () use ($product_id, $variation_id, $location_id, $delta, $product_variation_id) {
            // Load all rows for this key and consolidate to exactly one.
            $rows = VariationLocationDetails::where('variation_id', $variation_id)
                ->where('product_id', $product_id)
            ...
            if ($rows->isEmpty()) {
                ...
                $vld->product_id = $product_id;
                $vld->location_id = $location_id;
                if ($product_variation_id) {
                    $vld->product_variation_id = $product_variation_id;
                }
                $vld->qty_available = 0;
            } else {
                $vld = $rows->first();
                ...
                if ($rows->count() > 1) {
                    $sum = $rows->sum('qty_available');
                    // Delete all except first
                    VariationLocationDetails::whereIn('id', $rows->pluck('id')->slice(1)->all())->delete();
                    $vld->qty_available = $sum;
                }
                // Ensure product_variation_id is set if provided and empty
                if ($product_variation_id && empty($vld->product_variation_id)) {
                    $vld->product_variation_id = $product_variation_id;
                }
            }

            $vld->qty_available = ($vld->qty_available ?? 0) + $delta;
            $vld->save();
    ...
    /**
     * Determine if a transaction status affects stock levels
     *
     * @param  string  $status
     * @return bool
     */
    private function isStockAffectingStatus($status)
    {
    ...
    {
        // If changing from final/under processing to draft, return stock
        if ($this->isStockAffectingStatus($status_before) && $transaction->status == 'draft') {
        ...
        // If changing from draft to final/under processing, decrease stock
        elseif ($status_before == 'draft' && $this->isStockAffectingStatus($transaction->status)) {
            foreach ($input['products'] as $product) {
        ...
        // If transitioning between two stock-affecting statuses (final ↔ under processing)
        // DO NOT adjust stock for the status change itself, only for NEW products
        elseif ($this->isStockAffectingStatus($status_before) && $this->isStockAffectingStatus($transaction->status)) {
            // Stock was already adjusted when first entering a stock-affecting status
            // Only decrease stock for newly added products (those without transaction_sell_lines_id)
            foreach ($input['products'] as $product) {
                if (empty($product['transaction_sell_lines_id'])) {
                    $uf_quantity = $uf_data ? $this->num_uf($product['quantity']) : $product['quantity'];
                    $this->decreaseProductQuantity(
    ...
    public function addSingleProductOpeningStock($business_id, $product, $input, $transaction_date, $user_id)
    {
        ...
        foreach ($input as $key => $value) {
            ...
            if (array_key_exists($location_id, $locations)) {
                ...
                if ($qty > 0) {
                    $qty_formated = $this->num_f($qty, false, null, true);
    ...
    public function createOrUpdatePurchaseLines($transaction, $input_data, $currency_details, $enable_product_editing, $before_status = null)
    {
        ...
        foreach ($input_data as $data) {
            ...
            $new_quantity_f = $this->num_f($new_quantity, false, null, true);
            $old_qty = 0;
            ...
            $purchase_line->asked_qty = ! empty($data['asked_qty']) ? $this->num_uf($data['asked_qty']) : $new_quantity;
    ...
    {
        $new_quantity_f = $this->num_f($new_quantity, false, null, true);
        $old_qty = $this->num_f($old_quantity, false, null, true);
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        $products = $query->select(
            ...
            DB::raw("(SELECT SUM(TSL.quantity - TSL.quantity_returned) FROM transactions 
                  ...
                  WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell' AND transactions.location_id=vld.location_id
                  AND TSL.variation_id=variations.id) as total_sold"),
            DB::raw("(SELECT SUM(IF(transactions.type='sell_transfer', TSL.quantity, 0) ) FROM transactions 
                  JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                  WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell_transfer' AND transactions.location_id=vld.location_id AND (TSL.variation_id=variations.id)) as total_transfered"),
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        ...
        $sell_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->where('t.location_id', $location_id)
            // Treat 'under processing' like 'final' for stock summary
            ->whereIn('t.status', ['final', 'under processing'])
            ->where('p.business_id', $business_id)
            ->where('variations.id', $variation_id)
            ->select(
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin(
            ...
            // Ensure inflows appear before outflows on the same timestamp to avoid interim negatives
            ->orderByRaw("FIELD(transactions.type, 'opening_stock', 'purchase', 'purchase_return', 'sell_return', 'stock_adjustment', 'purchase_transfer', 'sell_transfer', 'production_purchase', 'production_sell', 'sell')")
        ...
        foreach ($stock_history as $stock_line) {
            ...
            if ($stock_line->transaction_type == 'sell') {
                // Include both 'final' and 'under processing' sells in history
                if (!in_array($stock_line->status, ['final', 'under processing'], true)) {
                    continue;
                }
                ...
                // Include both 'final' and 'under processing' sell transfers
                if (!in_array($stock_line->status, ['final', 'under processing'], true)) {
                    continue;
                }
                $quantity_change = -1 * $stock_line->sell_line_quantity;
                ...
                // Include both 'final' and 'under processing' production sells
                if (!in_array($stock_line->status, ['final', 'under processing'], true)) {
                    continue;
                }
                $quantity_change = -1 * $stock_line->sell_line_quantity;
    ...
    public function getVariationStockMisMatch($business_id, $variation_id, $location_id)
    {
        ...
        $stock_details = $query->select(
            DB::raw("(SELECT SUM(COALESCE(TSL.quantity, 0)) FROM transactions 
                    ...
                    WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell' AND transactions.location_id=$location_id 
                    AND TSL.variation_id=variations.id) as total_sold"),
            ...
            DB::raw("(SELECT SUM(COALESCE(TSL.quantity,0)) FROM transactions 
                    LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                    WHERE transactions.status IN ('final', 'under processing') AND transactions.type='sell_transfer' AND transactions.location_id=$location_id 
            ...
            DB::raw("(SELECT SUM(COALESCE(TSL.quantity, 0)) FROM transactions 
                    LEFT JOIN transaction_sell_lines AS TSL ON transactions.id=TSL.transaction_id
                    WHERE transactions.status IN ('final', 'under processing') AND transactions.type='production_sell' AND transactions.location_id=$location_id 
    ...
    // public function VinToYear($vin){
    //    // VIN Year Codes (2010 - 2029)
    //     $yearCodes = [
    ...
    //     // Validate VIN length
    //     if (strlen($vin) !== 17) {
    //         return "Invalid VIN length. Must be 17 characters.";
    //     }

    //     // Extract the 10th character
    //     $yearCode = strtoupper($vin[9]); 

    //     // Get model year
    //     return $yearCodes[$yearCode] ?? "Invalid VIN year code.";
    // }
