    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($models)
                ...
                ->editColumn('vin_model_code', function ($row) {
                

                    return $row->vin_model_code;
                })
                ...
                ->removeColumn('id')
                ->rawColumns(['action', 'vin_model_code', 'device_id', 'brand_id'])
                ->make(true);
        }
    }


public function importDeviceModels(Request $request)
{
    $request->validate([
    ...
    foreach ($rows as $row) {
        $name = $row[0] ?? null; // Extract first column (model name)
        $device_name = $row[1] ?? null; // Extract second column (device category name)

        if (!$name || !$device_name) {
            continue; // Skip empty rows
        }

        // Find device_id from categories table
        $device = Category::where('name', $device_name)->first();

        if (!$device) {
            $skipped++;
            continue; // Skip if device category doesn't exist
        }

        // Check if model already exists
        if (!DeviceModel::where('name', $name)->where('device_id', $device->id)->exists()) {
            DeviceModel::create([
                'name'        => $name,
                'business_id' => $user->business_id,
                'created_by'  => $user->id,
                'device_id'   => $device->id, // Assign found device_id
    ...
    if ($added > 0) {
        return back()->with('success', "$added models imported successfully! $skipped duplicates or missing categories skipped.");
    }

    return back()->with('error', 'No new models added. All were duplicates or had missing categories.');
}
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only('name', 'brand_id', 'device_id', 'repair_checklist','vin_model_code');
