...
<section class="content no-print">
    ...
    <input type="hidden" id="is_admin" value="{{ $is_user_admin ? 1 : 0 }}">
</section>
<!-- /.content -->
@stop
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function () {
            pending_job_sheets_datatable = $("#pending_job_sheets_table").DataTable({
                    ...
                    serverSide: true,
                    fixedHeader:false,
                        pageLength: 25, // <-- Set default page size here
                    ...
                    columns:[
                        ...
                        {
                            data: 'chassie_number', name: 'contact_device.chassis_number'
                        },
                        {
            ...
            completed_job_sheets_datatable = $("#completed_job_sheets_table").DataTable({
                    ...
                    columns:[
                        ...
                        {
                            data: 'plate_number', name: 'contact_device.plate_number', searchable: true
                        },
                        {
                            data: 'estimated_cost', name: 'estimated_cost'
                        },
            ...
            $(document).on('click', '.delete-job-sheet', function (e) {
                e.preventDefault();
                            ...
                            $.ajax({
                                ...
                                success: function(result) {
                                    if (result.success) {
                                        ...
                                        if (typeof pending_job_sheets_datatable !== 'undefined') {
                                            pending_job_sheets_datatable.ajax.reload();
                                        }
                                        if (typeof completed_job_sheets_datatable !== 'undefined') {
                                ...
                                error: function(xhr, status, error) {
                                    ...
                                    if (xhr.responseJSON && xhr.responseJSON.msg) {
                                        ...
                                        errorMsg = "{{ __('messages.unauthorized_action') }}";
                                    } else if (xhr.status === 404) {
                                        errorMsg = "{{ __('messages.not_found') }}";
                                    }
                            ...
                            $.ajax({
                                ...
                                error: function(xhr, status, error) {
                                    ...
                                    if (xhr.responseJSON && xhr.responseJSON.msg) {
                                        ...
                                        errorMsg = "{{ __('repair::lang.invalid_password') }}";
                                    }
                                    toastr.error(errorMsg);
                                    console.error('Hard delete error:', xhr, status, error);
                                }
                            });
                        }
                    });
