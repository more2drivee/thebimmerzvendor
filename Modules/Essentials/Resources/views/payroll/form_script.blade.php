...
$(document).ready( function () {
    ...
    function calculateTotal (id) {
        ...
        // ── Payment-cycle cap ────────────────────────────────────────────────
        // data-max-payable is set by the controller when the employee has already
        // received part of their salary this cycle (e.g. via an advance payroll).
        var $row         = $('tr[data-id="' + id + '"]');
        var maxPayable   = parseFloat($row.data('max-payable'));
        var alreadyPaid  = parseFloat($row.data('already-paid') || 0);
        var $capWarning  = $('#cap_warning_' + id);

        if (!isNaN(maxPayable) && maxPayable >= 0) {
            if (gross_amount > maxPayable) {
                gross_amount = maxPayable;
                if ($capWarning.length === 0) {
                    $row.find('td:last').append(
                        '<div id="cap_warning_' + id + '" class="alert alert-warning" ' +
                        'style="margin-top:6px;padding:5px 8px;font-size:11px;">' +
                        '<i class="fas fa-exclamation-triangle"></i> ' +
                        '<strong>Cycle cap applied.</strong> Already paid this cycle: ' +
                        __currency_trans_from_en(alreadyPaid, true) +
                        '. Max payable now: ' +
                        __currency_trans_from_en(maxPayable, true) +
                        '.</div>'
                    );
                }
            } else {
