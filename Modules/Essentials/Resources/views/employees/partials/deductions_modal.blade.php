...
<div class="row mb-3">
    <div class="col-md-12 text-right">
        <button type="button" class="btn btn-primary btn-sm add-deduction-btn" data-user-id="{{ $userId }}">
            ...
            <tr>
                ...
                <td>
                    @if($deduction->amount_type == 'percent')
                        {{ $deduction->amount }}%
                    @else
                        <span class="display_currency" data-currency_symbol="true">{{ $deduction->amount }}</span>
                    @endif
                </td>
                <td>
                ...
                <td>
                    @if($deduction->apply_on == 'next_payroll')
                        @lang('essentials::lang.next_payroll')
                    @elseif($deduction->apply_on == 'after_next')
                        @lang('essentials::lang.after_next')
                    @else
                        @lang('essentials::lang.every_payroll')
                    @endif
                </td>
                <td>{{ $deduction->start_date ? @format_date($deduction->start_date) : '-' }}</td>
                <td>{{ $deduction->end_date ? @format_date($deduction->end_date) : '-' }}</td>
                <td>
