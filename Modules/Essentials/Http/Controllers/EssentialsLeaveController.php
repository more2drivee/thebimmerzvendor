...
class EssentialsLeaveController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            $input = $request->only(['essentials_leave_type_id', 'start_date', 'end_date', 'reason',
                'employee_salary', 'per_day_salary', 'deduct_from_salary', 'leave_days_count',
                'deduction_amount', 'give_warning', 'warning_type', 'warning_note']);
    ...
    private function __addLeave($input, $user_id = null)
    {
        ...
        $input['leave_days_count'] = $leave_days;
        if (!empty($input['deduct_from_salary']) && $input['deduct_from_salary'] == 'yes' && !empty($input['per_day_salary'])) {
            $input['deduction_amount'] = $leave_days * floatval($input['per_day_salary']);
        } else {
        ...
        // Handle auto-absent / auto clock-out logic
        $this->handleAttendanceForLeave($input['user_id'], $input['business_id'], $input['start_date'], $input['end_date']);

        // Handle warning if applicable
        if (!empty($input['give_warning']) && $input['give_warning'] == 'yes') {
            $this->createEmployeeWarning($input['user_id'], $input['business_id'], $input);
        }
    ...
    /**
     * Handle attendance for leave - auto clock-out if clocked in, or mark as absent
     */
    private function handleAttendanceForLeave($user_id, $business_id, $start_date, $end_date)
    {
        $leave_start = \Carbon::parse($start_date);
        $leave_end = \Carbon::parse($end_date);
        
        // Iterate through each day of leave
        for ($date = $leave_start; $date->lte($leave_end); $date->addDay()) {
            $date_str = $date->format('Y-m-d');
            
            // Check if user has clocked in for this date
            $attendance = \Modules\Essentials\Entities\EssentialsAttendance::where('business_id', $business_id)
                ->where('user_id', $user_id)
                ->whereDate('clock_in_time', '=', $date_str)
                ->whereNull('clock_out_time')
                ->first();
            
            if ($attendance) {
                // User has clocked in - auto clock out at end of shift (default 5 PM)
                $clock_out_time = $date_str . ' 17:00:00';
                $attendance->clock_out_time = $clock_out_time;
                $attendance->save();
            }
            // If not clocked in, no action needed - user is absent (no attendance record)
        }
    }
     ...
     */
    private function createEmployeeWarning($user_id, $business_id, $leave_data)
    {
        $warning_data = [
            'business_id' => $business_id,
            'user_id' => $user_id,
            'issued_by' => request()->session()->get('user.id'),
            'warning_type' => $leave_data['warning_type'] ?? null,
            'reason' => $leave_data['warning_note'] ?? $leave_data['reason'] ?? null,
