...
class PayrollController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            if ($permitted_locations != 'all') {
                $payrolls->where(function ($q) use ($permitted_locations) {
                    $q->whereIn('epg.location_id', $permitted_locations)->orWhereNull(
                        'epg.location_id',
                    );
                });
            }
            ...
            return Datatables::of($payrolls)
                ->addColumn('action', function ($row) {
                    ...
                    $html .=
                        ...
                        '" data-container=".view_modal" class="btn-modal"><i class="fa fa-eye" aria-hidden="true"></i> ' .
                        __('messages.view') .
                        '</a></li>';
                    ...
                    if (
                        empty($row->payroll_group_id) &&
                        $row->payment_status != 'paid' &&
                        auth()->user()->can('essentials.create_payroll')
                    ) {
                        $html .=
                            '<li><a href="' .
                            action(
                                [
                                    \App\Http\Controllers\TransactionPaymentController::class,
                                    'addPayment',
                                ],
                                [$row->id],
                            ) .
                            '" class="add_payment_modal"><i class="fa fa-money"></i> ' .
        ...
        return view('essentials::payroll.index')->with(
            compact('employees', 'departments', 'designations', 'locations'),
        );
    }
    ...
    public function create()
    {
        ...
        $employees = User::where('business_id', $business_id)->find($employee_ids);

        // month_year string for salary advance matching (format: YYYY-MM — matches HTML input[type=month])
        $month_year_str = $year . '-' . str_pad($month, 2, '0', STR_PAD_LEFT);
        ...
        if (!empty($employees)) {
            ...
            foreach ($employees as $employee) {
                ...
                //get employee info
                $payrolls[$eid]['name'] = $employee->user_full_name;
                $payrolls[$eid]['essentials_salary'] = $employee->essentials_salary;
                $payrolls[$eid]['essentials_pay_period'] = $employee->essentials_pay_period;
                $payrolls[$eid]['essentials_pay_cycle'] = $employee->essentials_pay_cycle;

                // --- Initialize allowances and deductions arrays ---
                $payrolls[$eid]['allowances'] = [
                    'allowance_names' => [],
                    'allowance_amounts' => [],
                    'allowance_types' => [],
                    'allowance_percents' => [],
                ];
                $payrolls[$eid]['deductions'] = [
                    'deduction_names' => [],
                    'deduction_amounts' => [],
                    'deduction_types' => [],
                    'deduction_percents' => [],
                ...
                $payrolls[$eid][
                    'total_leaves'
                ] = $this->essentialsUtil->getTotalLeavesForGivenDateOfAnEmployee(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                ] = $this->essentialsUtil->getExpectedWorkingDays(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                ] = $this->essentialsUtil->getExpectedShiftHours(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                $payrolls[$eid]['holidays_count'] = $this->essentialsUtil->getHolidaysCount(
                    $business_id,
                    $start_date,
                    $end_date_str,
                    $location_id,
                );
                $payrolls[$eid]['leaves_summary'] = $this->essentialsUtil->getDeductedLeavesSummary(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                $payrolls[$eid][
                    'attendance_calendar'
                ] = $this->essentialsUtil->getAttendanceCalendarData(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                // --- Warnings count for the period ---
                $payrolls[$eid]['warnings_count'] = $this->essentialsUtil->getEmployeeWarningsCount(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                if ($commsn_calculation_type == 'payment_received') {
                    ...
                    $total_commission =
                        ($employee->cmmsn_percent * $sell_details['total_sales_with_commission']) /
                        100;
                }

                if ($total_commission > 0) {
                    $payrolls[$eid]['allowances']['allowance_names'][] = __(
                        'essentials::lang.sale_commission',
                    );
                    $payrolls[$eid]['allowances']['allowance_amounts'][] = $total_commission;
                    $payrolls[$eid]['allowances']['allowance_types'][] = 'fixed';
                    $payrolls[$eid]['allowances']['allowance_percents'][] = 0;
                }

                $settings = $this->essentialsUtil->getEssentialsSettings();
                //get total sales added by the employee
                $sale_totals = $this->transactionUtil->getUserTotalSales(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date,
                ...
                $total_sales =
                    ...
                    $settings['calculate_sales_target_commission_without_tax'] == 1
                        ? $sale_totals['total_sales_without_tax']
                        : $sale_totals['total_sales'];

                //get sales target if exists
                $sales_target = EssentialsUserSalesTarget::where('user_id', $eid)
                    ->where('target_start', '<=', $total_sales)
                    ->where('target_end', '>=', $total_sales)
                    ->first();
                $total_sales_target_commission_percent = !empty($sales_target)
                    ? $sales_target->commission_percent
                    : 0;
                $total_sales_target_commission = $this->transactionUtil->calc_percentage(
                    $total_sales,
                    $total_sales_target_commission_percent,
                );

                if ($total_sales_target_commission > 0) {
                    $payrolls[$eid]['allowances']['allowance_names'][] = __(
                        'essentials::lang.sales_target_commission',
                    );
                    $payrolls[$eid]['allowances'][
                        'allowance_amounts'
                    ][] = $total_sales_target_commission;
                    $payrolls[$eid]['allowances']['allowance_types'][] = 'fixed';
                    $payrolls[$eid]['allowances']['allowance_percents'][] = 0;
                }

                //get earnings & deductions of employee (system allowances/deductions)
                $allowances_and_deductions = $this->essentialsUtil->getEmployeeAllowancesAndDeductions(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date,
                ...
                foreach ($allowances_and_deductions as $ad) {
                    if ($ad->type == 'allowance') {
                        $payrolls[$eid]['allowances']['allowance_names'][] = $ad->description;
                        $payrolls[$eid]['allowances']['allowance_amounts'][] =
                            $ad->amount_type == 'fixed' ? $ad->amount : 0;
                        $payrolls[$eid]['allowances']['allowance_types'][] = $ad->amount_type;
                        $payrolls[$eid]['allowances']['allowance_percents'][] =
                            $ad->amount_type == 'percent' ? $ad->amount : 0;
                    } else {
                        $payrolls[$eid]['deductions']['deduction_names'][] = $ad->description;
                        $payrolls[$eid]['deductions']['deduction_amounts'][] =
                            $ad->amount_type == 'fixed' ? $ad->amount : 0;
                        $payrolls[$eid]['deductions']['deduction_types'][] = $ad->amount_type;
                        $payrolls[$eid]['deductions']['deduction_percents'][] =
                            $ad->amount_type == 'percent' ? $ad->amount : 0;
                    }
                }

                // --- Employee Bonuses (from essentials_employee_bonuses) ---
                $emp_bonuses = $this->essentialsUtil->getEmployeeBonusesForPayroll(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                ...
                $bonus_ids_to_apply = [];
                foreach ($emp_bonuses as $bonus) {
                    $bonus_amount = $bonus->amount;
                    $bonus_type = $bonus->amount_type; // fixed or percent
                    $bonus_percent = 0;

                    if ($bonus_type == 'percent') {
                        $bonus_percent = $bonus->amount;
                        $bonus_amount = 0; // will be calculated in JS based on salary
                    }

                    $payrolls[$eid]['allowances']['allowance_names'][] =
                        __('essentials::lang.bonus') . ': ' . $bonus->description;
                    $payrolls[$eid]['allowances']['allowance_amounts'][] = $bonus_amount;
                    $payrolls[$eid]['allowances']['allowance_types'][] = $bonus_type;
                    $payrolls[$eid]['allowances']['allowance_percents'][] = $bonus_percent;

                    // Track bonus IDs for marking as applied after payroll is saved
                    if ($bonus->apply_on == 'next_payroll' || $bonus->apply_on == 'after_next') {
                        $bonus_ids_to_apply[] = $bonus->id;
                    }
                }
                $payrolls[$eid]['bonus_ids_to_apply'] = $bonus_ids_to_apply;

                // === DEDUCTIONS ===

                // --- Employee Deductions (from essentials_employee_deductions) ---
                $emp_deductions = $this->essentialsUtil->getEmployeeDeductionsForPayroll(
                    $business_id,
                    $eid,
                    $start_date,
                    $end_date_str,
                );
                $deduction_ids_to_apply = [];
                foreach ($emp_deductions as $deduction) {
                    $ded_amount = $deduction->amount;
                    $ded_type = $deduction->amount_type;
                    $ded_percent = 0;

                    if ($ded_type == 'percent') {
                        $ded_percent = $deduction->amount;
                        $ded_amount = 0;
                    }

                    $payrolls[$eid]['deductions']['deduction_names'][] =
                        __('essentials::lang.deduction') . ': ' . $deduction->description;
                    $payrolls[$eid]['deductions']['deduction_amounts'][] = $ded_amount;
                    $payrolls[$eid]['deductions']['deduction_types'][] = $ded_type;
                    $payrolls[$eid]['deductions']['deduction_percents'][] = $ded_percent;

                    if (
                        $deduction->apply_on == 'next_payroll' ||
                        $deduction->apply_on == 'after_next'
                    ) {
                        $deduction_ids_to_apply[] = $deduction->id;
                    }
                }
                $payrolls[$eid]['deduction_ids_to_apply'] = $deduction_ids_to_apply;

                // --- Leave Deductions (from essentials_leaves where deduct_from_salary=yes) ---
                $leave_deductions = EssentialsLeave::where('business_id', $business_id)
                    ->where('user_id', $eid)
                    ->where('deduct_from_salary', 'yes')
                    ->where('status', 'approved')
                    ->where(function ($q) use ($start_date, $end_date_str) {
                        $q->whereBetween('start_date', [$start_date, $end_date_str])
                            ->orWhereBetween('end_date', [$start_date, $end_date_str])
                            ->orWhere(function ($q2) use ($start_date, $end_date_str) {
                                $q2->where('start_date', '<=', $start_date)->where(
                                    'end_date',
                                    '>=',
                                    $end_date_str,
                                );
                ...
                foreach ($leave_deductions as $leave) {
                    if ($leave->deduction_amount > 0) {
                        $payrolls[$eid]['deductions']['deduction_names'][] =
                            __('essentials::lang.leave_deduction') .
                            ' (' .
                            $leave->start_date .
                            ' - ' .
                            $leave->end_date .
                            ')';
                        $payrolls[$eid]['deductions']['deduction_amounts'][] =
                            $leave->deduction_amount;
                        $payrolls[$eid]['deductions']['deduction_types'][] = 'fixed';
                        $payrolls[$eid]['deductions']['deduction_percents'][] = 0;
                    }
                }

                // --- Salary Advances (approved, deduct_from_payroll matches this month) ---
                // Note: We track advance IDs to mark them as 'deducted' when payroll is saved,
                // but we DON'T add them as deduction lines anymore because they're already
                // counted in the "already paid" total (advance_payouts_paid).
                $salary_advances = $this->essentialsUtil->getSalaryAdvancesForPayroll(
                    $business_id,
                    $eid,
                    $month_year_str,
                );
                $advance_ids_to_deduct = [];
                foreach ($salary_advances as $advance) {
                    $advance_ids_to_deduct[] = $advance->id;
                }
                $payrolls[$eid]['advance_ids_to_deduct'] = $advance_ids_to_deduct;

                // ── Cycle info for display only (no restriction) ────────────
                // Query all payroll transactions for this employee in this month.
                // Uses whereYear+whereMonth so transactions on ANY day of the month are included.
                $previous_payrolls_this_cycle = Transaction::where('business_id', $business_id)
                    ->where('type', 'payroll')
                    ->where('expense_for', $eid)
                    ->where('status', 'final')
                    ->whereYear('transaction_date', $year)
                    ->whereMonth('transaction_date', $month)
                    ->orderBy('transaction_date')
                    ->get(['id', 'transaction_date', 'final_total', 'staff_note']);

                $total_already_paid = $previous_payrolls_this_cycle->sum('final_total');

                // Pass cycle info to blade for display — no salary override, no restriction
                $payrolls[$eid]['already_paid_this_cycle']      = $total_already_paid;
                $payrolls[$eid]['previous_payrolls_this_cycle'] = $previous_payrolls_this_cycle;
                $payrolls[$eid]['fully_paid']                   = false; // never block
            }

            $action = 'create';

            return view('essentials::payroll.create')->with(
                compact('month_name', 'transaction_date', 'year', 'payrolls', 'action', 'location'),
            );
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $payroll_group['gross_total'] = $this->transactionUtil->num_uf(
                $request->input('total_gross_amount'),
            );
            ...
            foreach ($payrolls as $key => $payroll) {
                $submitted_total = $this->moduleUtil->num_uf($payroll['final_total'] ?? 0);

                // Collect advance IDs regardless of whether we create a transaction
                if (!empty($payroll['advance_ids_to_deduct'])) {
                    $all_advance_ids = array_merge(
                        $all_advance_ids,
                        array_filter(explode(',', $payroll['advance_ids_to_deduct'])),
                    );
                }

                // Skip only if the gross is truly zero AND there are no allowances to save.
                // When salary is fully paid the user may still add allowances/deductions,
                // so we check the gross (final_total) which includes them.
                if ($submitted_total <= 0) {
                    continue; // nothing meaningful to save
                }

                // Collect IDs for post-save status updates
                if (!empty($payroll['bonus_ids_to_apply'])) {
                    $all_bonus_ids = array_merge(
                        $all_bonus_ids,
                        explode(',', $payroll['bonus_ids_to_apply']),
                    );
                }
                if (!empty($payroll['deduction_ids_to_apply'])) {
                    $all_deduction_ids = array_merge(
                        $all_deduction_ids,
                        explode(',', $payroll['deduction_ids_to_apply']),
                    );
                }
                if (!empty($payroll['advance_ids_to_deduct'])) {
                    $all_advance_ids = array_merge(
                        $all_advance_ids,
                        explode(',', $payroll['advance_ids_to_deduct']),
                    );
                }
                ...
                $payroll['essentials_amount_per_unit_duration'] = $this->moduleUtil->num_uf(
                    $payroll['essentials_amount_per_unit_duration'],
                );

                $allowances_and_deductions = $this->getAllowanceAndDeductionJson($payroll);
                $payroll['essentials_allowances'] =
                    $allowances_and_deductions['essentials_allowances'];
                $payroll['essentials_deductions'] =
                    $allowances_and_deductions['essentials_deductions'];
                ...
                if (empty($payroll['ref_no'])) {
                    ...
                    $prefix = !empty($settings['payroll_ref_no_prefix'])
                        ...
                        : '';
                    $payroll['ref_no'] = $this->moduleUtil->generateReferenceNumber(
                        'payroll',
                        $ref_count,
                        null,
                        $prefix,
                ...
                unset(
                    ...
                    $payroll['total'],
                    $payroll['bonus_ids_to_apply'],
                    $payroll['deduction_ids_to_apply'],
                    $payroll['advance_ids_to_deduct'],
                );
            ...
            // Mark one-time bonuses as applied
            if (!empty($all_bonus_ids)) {
                EssentialsEmployeeBonus::whereIn('id', $all_bonus_ids)
                    ->whereIn('apply_on', ['next_payroll', 'after_next'])
                    ->update(['status' => 'applied']);
            }

            // Mark one-time deductions as applied
            if (!empty($all_deduction_ids)) {
                EssentialsEmployeeDeduction::whereIn('id', $all_deduction_ids)
                    ->whereIn('apply_on', ['next_payroll', 'after_next'])
                    ->update(['status' => 'applied']);
            }

            // Mark salary advances as deducted
            if (!empty($all_advance_ids)) {
                EssentialsSalaryAdvance::whereIn('id', $all_advance_ids)->update([
                    'status' => 'deducted',
                ]);
            }
    ...
    private function getAllowanceAndDeductionJson($payroll)
    {
        ...
        foreach ($payroll['allowance_amounts'] as $key => $value) {
            if (!empty($allowance_names[$key])) {
                ...
                $allowance_percent_array[] = !empty($allowance_percents[$key])
                    ? $this->moduleUtil->num_uf($allowance_percents[$key])
                    : 0;
            }
        }
        ...
        foreach ($payroll['deduction_amounts'] as $key => $value) {
            if (!empty($deduction_names[$key])) {
                ...
                $deduction_percents_array[] = !empty($deduction_percents[$key])
                    ? $this->moduleUtil->num_uf($deduction_percents[$key])
                    : 0;
            }
        }
    ...
    public function show($id)
    {
        ...
        $query = Transaction::where('business_id', $business_id)->with([
            'transaction_for',
            'payment_lines',
        ]);
        ...
        $department = Category::where('category_type', 'hrm_department')->find(
            $payroll->transaction_for->essentials_department_id,
        );

        $designation = Category::where('category_type', 'hrm_designation')->find(
            $payroll->transaction_for->essentials_designation_id,
        );

        $location = BusinessLocation::where('business_id', $business_id)->find(
            $payroll->transaction_for->location_id,
        );

        $month_name = $transaction_date->format('F');
        $year = $transaction_date->format('Y');
        $allowances = !empty($payroll->essentials_allowances)
            ? json_decode($payroll->essentials_allowances, true)
            : [];
        $deductions = !empty($payroll->essentials_deductions)
            ? json_decode($payroll->essentials_deductions, true)
            : [];
        ...
        return view('essentials::payroll.show')->with(
            compact(
                'payroll',
                'month_name',
                'allowances',
    ...
    public function edit($id)
    {
        ...
        if (
            ...
            !auth()->user()->can('essentials.update_payroll')
        ) {
            abort(403, 'Unauthorized action.');
        }
        ...
        return view('essentials::payroll.edit')->with(
            compact('payroll', 'month_name', 'allowances', 'deductions', 'year'),
        );
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $input['essentials_amount_per_unit_duration'] = $this->moduleUtil->num_uf(
                $input['essentials_amount_per_unit_duration'],
            );
    ...
    public function destroy($id)
    {
        ...
        if (
            ...
            !auth()->user()->can('essentials.delete_payroll')
        ) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if (request()->ajax()) {
            try {
                ...
                if ($payroll_group->status == 'draft') {
                    $transaction_ids = $payroll_group->payrollGroupTransactions
                        ->pluck('id')
                        ->toArray();
    ...
    public function payrollGroupDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            $payroll_groups = PayrollGroup::where(
                'essentials_payroll_groups.business_id',
                $business_id,
            )
                ->join('users as u', 'u.id', '=', 'essentials_payroll_groups.created_by')
                ->leftJoin(
                    'business_locations as BL',
                    'essentials_payroll_groups.location_id',
                    '=',
                    'BL.id',
                )
                ->select(
                ...
                ->addColumn('action', function ($row) {
                    ...
                    if (
                        ...
                        $html .=
                            ...
                            '" class="delete-payroll"><i class="fa fa-trash" aria-hidden="true"></i> ' .
                            __('messages.delete') .
                            '</a></li>';
                    }
    ...
    public function viewPayrollGroup($id)
    {
        ...
        $payroll_group = PayrollGroup::where('business_id', $business_id)
            ->with([
                'payrollGroupTransactions',
                'payrollGroupTransactions.transaction_for',
                'businessLocation',
                'business',
            ])
            ->findOrFail($id);
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            //get employee info
            $payrolls[$transaction->expense_for]['employee'] =
                $transaction->transaction_for->user_full_name;
            $payrolls[$transaction->expense_for]['bank_details'] = json_decode(
                $transaction->transaction_for->bank_details,
                true,
            );
        }

        return view('essentials::payroll.view_payroll_group')->with(
            compact('payroll_group', 'month_name', 'year', 'payrolls'),
        );
    }
    ...
    public function getEditPayrollGroup($id)
    {
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            //get employee info
            $payrolls[$transaction->expense_for]['name'] =
                $transaction->transaction_for->user_full_name ?? '';
            $payrolls[$transaction->expense_for]['staff_note'] = $transaction->staff_note;
            $payrolls[$transaction->expense_for]['essentials_amount_per_unit_duration'] =
                $transaction->essentials_amount_per_unit_duration;
            $payrolls[$transaction->expense_for]['essentials_duration'] =
                $transaction->essentials_duration;
            $payrolls[$transaction->expense_for]['essentials_duration_unit'] =
                $transaction->essentials_duration_unit;
            $payrolls[$transaction->expense_for][
                'total_leaves'
            ] = $this->essentialsUtil->getTotalLeavesForGivenDateOfAnEmployee(
                $business_id,
                $transaction->expense_for,
            ...
            $allowances = !empty($transaction->essentials_allowances)
                ? json_decode($transaction->essentials_allowances, true)
                : [];
            ...
            $deductions = !empty($transaction->essentials_deductions)
                ? json_decode($transaction->essentials_deductions, true)
                : [];
    ...
    public function getUpdatePayrollGroup(Request $request)
    {
        ...
        try {
            ...
            $pg_input['gross_total'] = $this->transactionUtil->num_uf(
                $request->input('total_gross_amount'),
            );

            DB::beginTransaction();
            $payroll_group = PayrollGroup::where('business_id', $business_id)->findOrFail(
                $payroll_group_id,
            );
            ...
            foreach ($payrolls as $key => $payroll) {
                ...
                if (!empty($payroll_trans)) {
                    ...
                    if ($notify_employee && $payroll_group->status == 'final') {
                        ...
                        $payroll_trans->transaction_for->notify(
                            new PayrollNotification($payroll_trans),
                        );
                    }
                }
            }
    ...
    public function addPayment($id)
    {
        ...
        foreach ($payroll_group->payrollGroupTransactions as $transaction) {
            ...
            $payrolls[$transaction->expense_for]['amount_formated'] = $this->transactionUtil->num_f(
                $pending_amount,
            );
            $payrolls[$transaction->expense_for]['payments'] = TransactionPayment::where(
                'transaction_id',
                $transaction->id,
            )->get();
        ...
        return view('essentials::payroll.pay_payroll_group')->with(
            compact('payroll_group', 'month_name', 'year', 'payrolls', 'payment_types', 'accounts'),
        );
    }
    ...
    public function postAddPayment(Request $request)
    {
        ...
        try {
            ...
            foreach ($payments as $employee_id => $payment) {
                $transaction = Transaction::where('business_id', $business_id)->findOrFail(
                    $payment['transaction_id'],
                );
                $transaction_before = $transaction->replicate();
                if (
                    $transaction->payment_status != 'paid' &&
                    !empty($payment['final_total']) &&
                    ...
                    DB::beginTransaction();
                    $ref_count = $this->transactionUtil->setAndGetReferenceCount(
                        'purchase_payment',
                    );
                    // Generate reference number
                    $input['payment_ref_no'] = $this->transactionUtil->generateReferenceNumber(
                        'purchase_payment',
                        $ref_count,
                    );
    ...
    protected function _updatePayrollGroupPaymentStatus($payroll_group_id, $business_id)
    {
        ...
        $total_paid = $payroll_group->payrollGroupTransactions
            ->where('payment_status', 'paid')
            ->count();
        $total_due = $payroll_group->payrollGroupTransactions
    ...
    public function getMyPayrolls(Request $request)
    {
            ...
            return Datatables::of($payrolls)
                ->addColumn('action', function ($row) {
                    $html =
                        ...
                        '" data-container=".view_modal" class="btn-modal btn-info btn btn-sm">
                            <i class="fa fa-eye" aria-hidden="true"></i> ' .
                        __('messages.view') .
                        '</a>';
        ...
        return view('essentials::payroll.partials.user_payrolls')->with(compact('pay_components'));
    }
    ...
    /**
     * Get employee salary data for leave form
     */
    public function getEmployeeSalary($user_id)
    {
        $business_id = request()->session()->get('user.business_id');
        ...
        if ($user) {
            return response()->json([
                'salary' => $user->essentials_salary ?? 0,
                'pay_period' => $user->essentials_pay_period ?? 'month',
            ]);
        }
