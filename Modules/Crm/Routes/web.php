...
use Modules\Crm\Http\Controllers\ContactBookingController;
use Modules\Crm\Http\Controllers\LoginController;
...
Route::middleware('web', 'authh', 'SetSessionData', 'auth', 'language', 'timezone', 'ContactSidebarMenu', 'CheckContactLogin')->prefix('contact')->name('contact.')->group(function () {
    Route::get('add-booking', [ContactBookingController::class, 'contactBooking'])->name('contactBooking');
    ...
    Route::get('bookings/get-models/{brandId}', [BookingController::class, 'getModelsByBrand']);
    Route::post('bookings/storeBooking', [ContactBookingController::class, 'storebooking']);
});
...
Route::middleware('web', 'authh', 'auth', 'SetSessionData', 'language', 'timezone', 'AdminSidebarMenu', 'CheckUserLogin')->prefix('crm')->group(function () {
    ...
    // Contact devices routes
    Route::get('contact-devices/{contact_id}', [Modules\Crm\Http\Controllers\LeadController::class, 'getContactDevices']);
    Route::post('contact-devices', [Modules\Crm\Http\Controllers\LeadController::class, 'storeContactDevice']);
    Route::get('contact-device/{id}', [Modules\Crm\Http\Controllers\LeadController::class, 'getContactDevice']);
    Route::put('contact-device/{id}', [Modules\Crm\Http\Controllers\LeadController::class, 'updateContactDevice']);
    Route::delete('contact-device/{id}', [Modules\Crm\Http\Controllers\LeadController::class, 'destroyContactDevice']);
    Route::get('get-models-for-brand/{brand_id}', [Modules\Crm\Http\Controllers\LeadController::class, 'getModelsForBrand']);
    ...
    Route::get('lead/{lead_id}/followup-log', [Modules\Crm\Http\Controllers\LeadController::class, 'showFollowupLog']);

    // Estimator details for CRM (web guard), used by leads modal
    Route::get('estimator/{id}/details', [Modules\Crm\Http\Controllers\LeadController::class, 'getEstimatorDetails']);

    // Smart Maintenance Predictions
    Route::get('service-predictions', [Modules\Crm\Http\Controllers\ServicePredictionController::class, 'index']);
    Route::get('service-predictions/history/{contact_id}/{device_id}', [Modules\Crm\Http\Controllers\ServicePredictionController::class, 'customerHistory']);
    Route::post('service-predictions/send-reminder', [Modules\Crm\Http\Controllers\ServicePredictionController::class, 'sendReminder']);
    Route::post('service-predictions/recalculate', [Modules\Crm\Http\Controllers\ServicePredictionController::class, 'recalculate']);
});
