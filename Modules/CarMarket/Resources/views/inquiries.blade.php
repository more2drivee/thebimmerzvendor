@extends('layouts.app')

@section('title', __('carmarket::lang.inquiries') . ' - ' . __('carmarket::lang.module_title'))

@section('javascript')
<script>
$(document).ready(function() {
    var inquiriesTable = $('#inquiries-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('carmarket.inquiries.datatables') }}",
            data: function(d) {
                d.status = $('#filter_inq_status').val();
            }
        },
        columns: [
            { data: 'id', name: 'id', orderable: true },
            { data: 'vehicle_title', name: 'vehicle_title', orderable: false },
            { data: 'buyer_name', name: 'buyer_name', orderable: false },
            { data: 'buyer_phone', name: 'buyer_phone', orderable: false },
            { data: 'inquiry_type', name: 'inquiry_type', orderable: false },
            { data: 'message', name: 'message', orderable: false },
            { data: 'offered_price', name: 'offered_price', orderable: true,
                render: function(data) {
                    return data ? parseFloat(data).toLocaleString() : '-';
                }
            },
            { data: 'action', name: 'action', orderable: false, searchable: false },
            { data: 'created_at', name: 'created_at', orderable: true }
        ],
        order: [[0, 'desc']],
        pageLength: 25
    });

    $('#filter_inq_status').change(function() {
        inquiriesTable.ajax.reload();
    });

    // Update inquiry status
    $(document).on('change', '.inquiry-status-select', function() {
        var id = $(this).data('id');
        var status = $(this).val();
        $.ajax({
            url: "{{ url('carmarket/inquiries') }}/" + id + "/status",
            method: 'PUT',
            data: { _token: '{{ csrf_token() }}', status: status },
            success: function(res) {
                if (res.success) toastr.success('Updated');
                else toastr.error('Error');
            }
        });
    });
});
</script>
@endsection

@section('content')
@include('carmarket::layouts.nav')

<section class="content-header no-print">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">
        @lang('carmarket::lang.inquiries')
    </h1>
</section>

<section class="content no-print">
    <div class="tw-bg-white tw-rounded-2xl tw-border tw-border-gray-200 tw-p-4 md:tw-p-6 tw-mb-4">
        <div class="row">
            <div class="col-md-3">
                <div class="form-group">
                    <label>@lang('carmarket::lang.inquiry_status')</label>
                    <select id="filter_inq_status" class="form-control">
                        <option value="">@lang('carmarket::lang.all_statuses')</option>
                        @foreach(['new','contacted','negotiating','closed_won','closed_lost'] as $s)
                            <option value="{{ $s }}">{{ ucfirst(str_replace('_', ' ', $s)) }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        </div>
    </div>

    <div class="tw-bg-white tw-rounded-2xl tw-border tw-border-gray-200 tw-p-4 md:tw-p-6">
        <div class="table-responsive">
            <table class="table table-bordered table-striped table-hover" id="inquiries-table">
                <thead>
                    <tr>
                        <th width="50">{{ __('messages.id') }}</th>
                        <th>@lang('carmarket::lang.vehicle')</th>
                        <th>@lang('carmarket::lang.buyer_name')</th>
                        <th>@lang('carmarket::lang.seller_phone')</th>
                        <th>@lang('carmarket::lang.inquiry_type')</th>
                        <th>@lang('carmarket::lang.message')</th>
                        <th>@lang('carmarket::lang.offered_price')</th>
                        <th width="150">@lang('carmarket::lang.inquiry_status')</th>
                        <th>{{ __('messages.date') }}</th>
                    </tr>
                </thead>
            </table>
        </div>
    </div>
</section>
@endsection
