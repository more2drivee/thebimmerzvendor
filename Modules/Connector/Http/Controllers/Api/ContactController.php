...
class ContactController extends ApiController
{
    ...
    /**
     * Quick create contact with vehicle
     *
     * @bodyParam name string required Contact name Example: Ahmed Ali
     * @bodyParam mobile string required Mobile number Example: 01001234567
     * @bodyParam type string Contact type (default customer) Example: customer
     * @bodyParam category_id integer required Vehicle brand/category ID Example: 12
     * @bodyParam model_id integer required Vehicle model ID Example: 345
     * @bodyParam color string required Vehicle color Example: Red
     * @bodyParam plate_number string required Plate number Example: ABC-1234
     * @bodyParam manufacturing_year string required Year Example: 2021
     * @bodyParam car_type string required Car type Example: Sedan
     * @bodyParam chassis_number string Chassis/VIN Example: WDBRF40J33F123456
     * @bodyParam motor_cc integer Engine displacement (cc), nullable Example: 1600
     *
     * @response {"data": {"id": 10, "name": "Ahmed Ali", "mobile": "01001234567"}}
     */
    public function quickStore(Request $request)
    {
        try {
            ...
            // Create contact with minimal fields (use name, not first_name)
            ...
            // Create associated contact device (vehicle)
            DB::table('contact_device')->insert([
                'device_id' => (int) $request->input('category_id'),
                'models_id' => (int) $request->input('model_id'),
                'color' => $request->input('color'),
                'chassis_number' => $request->input('chassis_number'),
                'plate_number' => $request->input('plate_number'),
                'manufacturing_year' => (string) $request->input('manufacturing_year'),
                'car_type' => $request->input('car_type'),
                'contact_id' => $contact->id,
                'brand_origin_variant_id' => $request->input('brand_origin_variant_id'),
                'motor_cc' => $request->input('motor_cc'),
            ]);

            DB::commit();

            return new CommonResource($contact);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::emergency('File:'.$e->getFile().'Line:'.$e->getLine().'Message:'.$e->getMessage());
            return $this->otherExceptions($e);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            if ($request->has('motor_cc')) {
                DB::table('contact_device')
                    ->where('contact_id', $id)
                    ->update(['motor_cc' => $request->input('motor_cc')]);
            }
    ...
    /**
     * Find contacts by VIN / chassis number.
     *
     * @queryParam vin string required VIN / chassis number of the vehicle Example: WDBRF40J33F123456
     *
     * @response {
     *   "data": [
     *     {
     ...
     *       "car_type": "Sedan",
     *       "chassis_number": "WDBRF40J33F123456"
     *     }
     *   ]
     * }
     */
    public function contactsByVin(Request $request)
    {
        $request->validate([
            'vin' => 'required|string',
        ]);
        ...
        $results = DB::table('contacts')
            ...
            ->where('contact_device.chassis_number', $vin)
            ->select(
                'contacts.id',
                'contacts.name',
                'contacts.first_name',
