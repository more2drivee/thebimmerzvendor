...
class CommonResourceController extends ApiController
{
     ...
     *
     * @queryParam location_id optional id of the location to get payment methods for
     *
     * @response {
        "cash": "Cash",
        "card": "Card",
    ...
    public function getPaymentMethods()
    {
        ...
        // Use the Util's payment_types which handles custom labels and location settings
        $payment_methods = $this->commonUtil->payment_types($location, false, $business_id);
     ...
     *
     * @response {
        "about_us": "Your business description here..."
    }
     */
    public function getAboutUs()
    {
        ...
        return [
            'about_us' => $common_settings['about_us'] ?? '',
        ];
    }

    /**
     * Get customer loyalty points balance and redemption details
     *
     * @queryParam contact_id required Customer contact ID Example: 1
     * @response {
        "data": {
            "total_points": 150,
            "redeemable_points": 100,
            "redeemable_amount": 50.00,
            "points_used": 50,
            "points_expired": 0,
            "enable_rp": true,
            "rp_name": "Points",
            "min_redeem_point": 10,
            "max_redeem_point": 100,
            "amount_per_point": 0.50,
            "min_order_total_for_redeem": 100.00
        }
    }
     */
    public function getCustomerLoyaltyPoints()
    {
        ...
        if ($business->enable_rp != 1) {
            return response()->json([
                'success' => false,
                'message' => 'Loyalty points are not enabled for this business.',
            ], 400);
        }
        ...
        $redeem_details = $this->transactionUtil->getRewardRedeemDetails($business_id, $contact_id);

        return response()->json([
            'success' => true,
            'data' => [
                'total_points' => $contact->total_rp ?? 0,
                'redeemable_points' => $redeem_details['points'],
                'redeemable_amount' => $redeem_details['amount'],
                'points_used' => $contact->total_rp_used ?? 0,
                'points_expired' => $contact->total_rp_expired ?? 0,
                'enable_rp' => true,
                'rp_name' => $business->rp_name,
                'min_redeem_point' => $business->min_redeem_point,
                'max_redeem_point' => $business->max_redeem_point,
                'amount_per_point' => $business->redeem_amount_per_unit_rp,
                'min_order_total_for_redeem' => $business->min_order_total_for_redeem,
            ],
        ]);
    }
     ...
     *
     * @bodyParam contact_id required Customer contact ID Example: 1
     * @bodyParam points_to_redeem required Number of points to redeem Example: 50
     * @bodyParam order_total required Order total amount Example: 200.00
     * @response {
        "success": true,
        "message": "Points redeemed successfully.",
        "data": {
            "points_redeemed": 50,
            "amount_discounted": 25.00,
    ...
    public function redeemLoyaltyPoints(Request $request)
    {
        ...
        if ($validated['points_to_redeem'] > $redeem_details['points']) {
            return response()->json([
                'success' => false,
                'message' => 'Requested points exceed available redeemable points.',
            ], 400);
        }

        if ($business->min_order_total_for_redeem > $validated['order_total']) {
            return response()->json([
                'success' => false,
                'message' => 'Order total must be at least ' . $business->min_order_total_for_redeem . ' to redeem points.',
            ], 400);
        }

        $amount_per_point = $business->redeem_amount_per_unit_rp;
        $discounted_amount = $validated['points_to_redeem'] * $amount_per_point;

        if ($discounted_amount > $validated['order_total']) {
            return response()->json([
                'success' => false,
                'message' => 'Discounted amount cannot exceed order total.',
            ], 400);
        }

        $loyalty_request = \App\LoyaltyDiscountRequest::create([
            'business_id' => $business_id,
            'contact_id' => $contact->id,
            'points_requested' => $validated['points_to_redeem'],
        ...
        return response()->json([
            'success' => true,
            'message' => 'Loyalty discount request created successfully. Waiting for approval.',
            'data' => [
                'request_id' => $loyalty_request->id,
                'points_requested' => $validated['points_to_redeem'],
                'amount_discounted' => $discounted_amount,
                'status' => 'pending',
            ],
