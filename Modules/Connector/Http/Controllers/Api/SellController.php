...
class SellController extends ApiController
{
    ...
    public function __construct(
        ...
        $this->stockSyncUtil = $stockSyncUtil;
    ...
    public function index()
    {
        ...
        $is_customer = !empty($user->crm_contact_id);

        if (! $is_admin && ! $is_customer && ! auth()->user()->hasAnyPermission(['sell.view', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
        ...
        $with = ['sell_lines.product', 'payment_lines', 'contact', 'job_sheet.maintenance_notes'];
        $query = Transaction::where('business_id', $business_id)
                            ->where('type', 'sell')
                            ->with(['job_sheet' => function($q) {
                                $q->with(['maintenance_notes' => function($q) {
                                    $q->where('category_status', 'note')
                                      ->with(['creator:id,first_name,last_name,surname', 'repairStatus:id,name,color'])
        ...
        if ($is_customer) {
            $query->where('transactions.contact_id', $user->crm_contact_id);
        } elseif (! $user->can('direct_sell.view')) {
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            foreach ($sells as $sell_data) {
                ...
                foreach ($sell_data['products'] as &$product) {
                    ...
                    // Use StockSyncUtil for atomic sell line creation + VLD + mapping
                    ...
                    $this->stockSyncUtil->processFullSellLines(
                        $transaction,
                        $input['products'],
                        $input['location_id'],
                        null, // statusBefore is null for new transactions
                        $business_info,
                        true, // isNew
                        [],
                        true // ufData - apply num_uf where needed
                    );

                    // Refresh transaction totals after sync
                    $transaction->refresh();
                    ...
                    if ($input['status'] == 'final' || $input['status'] == 'under processing') {
    ...
    private function __formatSellData($data, $transaction = null)
    {
        ...
        if (! empty($data['products'])) {
            foreach ($data['products'] as $product_data) {
                $sell_line = null;
                $sell_line_id = $product_data['sell_line_id'] ?? $product_data['transaction_sell_lines_id'] ?? null;
     ...
     * 
     * @bodyParam clock_in_time string optional Clock in time (format: Y-m-d H:i:s). Default: now
     * @bodyParam note string optional Clock in note
     * @bodyParam ip_address string optional IP address
     *
     * @response {
     *     "success": true,
     ...
     *         "id": 1,
     *         "user_id": 5,
     *         "clock_in_time": "2026-02-23 08:00:00",
    ...
    public function clockIn()
    {
        ...
        $clockInTime = $data['clock_in_time'] ?? \Carbon\Carbon::now();
        $date = \Carbon\Carbon::parse($clockInTime)->format('Y-m-d');

        // Check if there's an active shift today (clocked in but not clocked out)
        $activeAttendance = \Modules\Essentials\Entities\EssentialsAttendance::where('user_id', $user->id)
            ->where('business_id', $businessId)
            ->whereDate('clock_in_time', $date)
            ->whereNull('clock_out_time')
            ->first();

        // If there's an active shift, user must clock out first
        if ($activeAttendance) {
            return response()->json([
                'success' => false,
                'error' => 'You have an active shift. Please clock out first before clocking in again.',
                'active_record' => [
                    'id' => $activeAttendance->id,
                    'clock_in_time' => $activeAttendance->clock_in_time,
                    'clock_out_time' => $activeAttendance->clock_out_time,
                ]
            ], 400);
        }

        // Create attendance record (allows multiple clock-ins per day after clocking out)
        $attendance = \Modules\Essentials\Entities\EssentialsAttendance::create([
            'user_id' => $user->id,
     ...
     * 
     * @bodyParam clock_out_time string optional Clock out time (format: Y-m-d H:i:s). Default: now
     * @bodyParam note string optional Clock out note
     *
     * @response {
     *     "success": true,
     ...
     *         "user_id": 5,
     *         "clock_in_time": "2026-02-23 08:00:00",
     *         "clock_out_time": "2026-02-23 17:00:00",
    ...
    public function clockOut()
    {
        ...
        $clockOutTime = $data['clock_out_time'] ?? \Carbon\Carbon::now();
        $date = \Carbon\Carbon::parse($clockOutTime)->format('Y-m-d');

        // Find today's attendance record
        $attendance = \Modules\Essentials\Entities\EssentialsAttendance::where('user_id', $user->id)
            ->where('business_id', $businessId)
            ->whereDate('clock_in_time', $date)
            ->whereNull('clock_out_time')
            ->first();
        ...
        $workedHours = \Carbon\Carbon::parse($attendance->clock_in_time)
            ->diffInHours(\Carbon\Carbon::parse($attendance->clock_out_time));
    ...
    /**
     * Get shift statistics for authenticated user
     * 
     * Returns totals for invoices, payments, and returns within the current shift (clock in to clock out)
     *
     * @response {
     *     "success": true,
     *     "data": {
     ...
     * }
     */
    public function shiftStats()
    {
        $user = Auth::user();
        $business_id = $user->business_id;

        // Get today's attendance record for the user - get the latest one (with or without clock out)
        $attendance = \Modules\Essentials\Entities\EssentialsAttendance::where('user_id', $user->id)
        ...
        // If clock out exists, use it. If not, use current time
        $clockInTime = $attendance->clock_in_time;
        $clockOutTime = $attendance->clock_out_time ?? \Carbon\Carbon::now();

        // Get permitted locations
        $permittedLocations = $user->permitted_locations($business_id);

        // Base query for transactions in shift time range
        $baseQuery = function ($type) use ($business_id, $user, $permittedLocations, $clockInTime, $clockOutTime) {
            $query = Transaction::where('business_id', $business_id)
                ->where('type', $type)
                ->where('status', 'final')
                ->where('created_by', $user->id)
                ->where('transaction_date', '>=', $clockInTime)
                ->where('transaction_date', '<=', $clockOutTime);

            if ($permittedLocations != 'all') {
                $query->whereIn('location_id', $permittedLocations);
            }

            return $query;
        };

        // Invoices (sell transactions)
        $invoicesQuery = $baseQuery('sell');
        $invoicesTotal = (clone $invoicesQuery)->sum('final_total');
        $invoicesCount = (clone $invoicesQuery)->count();

        // Returns (sell_return transactions)
        $returnsQuery = $baseQuery('sell_return');
        $returnsTotal = (clone $returnsQuery)->sum('final_total');
        $returnsCount = (clone $returnsQuery)->count();

        // Payments - get all payments for sell transactions in shift
        $sellIds = (clone $invoicesQuery)->pluck('id');
        
        $payments = TransactionPayment::whereIn('transaction_id', $sellIds)
            ->where('created_by', $user->id)
            ->where('paid_on', '>=', $clockInTime)
            ->where('paid_on', '<=', $clockOutTime)
            ->where('is_return', 0)
            ->selectRaw('method, SUM(amount) as total_amount, COUNT(*) as count')
            ->groupBy('method')
            ->get();

        // Build dynamic payment methods array
        $paymentMethods = [];
        $totalPayments = 0;
        $paymentsCount = 0;
        ...
        foreach ($payments as $payment) {
            ...
            $paymentMethods[$methodName] = [
                'total' => round($payment->total_amount, 2),
                'count' => $payment->count,
            ];
            $totalPayments += $payment->total_amount;
            $paymentsCount += $payment->count;
        }

        // Grand total = Invoices - Returns
        $grandTotal = $invoicesTotal - $returnsTotal;
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'shift' => [
                    'clock_in_time' => $clockInTime,
                    'clock_out_time' => $attendance->clock_out_time,
                    'is_active' => $attendance->clock_out_time === null,
                ],
                // 1. اجمالي الفواتير (Total Invoices)
                'total_invoices' => round($invoicesTotal, 2),
                // 2. عدد الفواتير (Number of Invoices)
                'invoices_count' => $invoicesCount,
                // 3. Dynamic payment methods
                'payment_methods' => $paymentMethods,
                // 4. اجمالي المرتجعات (Total Returns)
                'total_returns' => round($returnsTotal, 2),
                // 5. عدد المرتجعات (Number of Returns)
                'returns_count' => $returnsCount,
                // 6. اجمالي المدفوعات (Total Payments)
                'total_payments' => round($totalPayments, 2),
                // 7. عدد المدفوعات (Number of Payments)
                'payments_count' => $paymentsCount,
                // 8. الإجمالي (Grand Total = Invoices - Returns)
