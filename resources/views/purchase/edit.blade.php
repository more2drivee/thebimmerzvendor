  ...
  <input type="hidden" id="purchase_id" value="{{ $purchase->id }}">
    ...
    @component('components.widget', ['class' => 'box-primary'])
        <div class="row">
            ...
            @if(!empty($purchase->repair_job_sheet_id) || !empty($purchase->invoice_ref))
            <div class="row">
              <div class="col-sm-4">
                <div class="form-group">
                  {!! Form::label('repair_job_sheet_id', __('repair::lang.job_sheet_no') . ':') !!}
                  {!! Form::select('repair_job_sheet_id', [], $purchase->repair_job_sheet_id, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'id' => 'repair_job_sheet_id']); !!}
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-sm-4">
                <div class="form-group">
                  {!! Form::label('repair_job_sheet_id', __('repair::lang.job_sheet_no') . ':') !!}
                  {!! Form::select('repair_job_sheet_id', [], null, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'id' => 'repair_job_sheet_id']); !!}
                </div>
              </div>
            </div>
            @endif
            ...
            <div class="col-sm-12">
              ...
              <div class="pull-right col-md-5">
                <table class="pull-right col-md-12">
                  ...
                  <tr>
                    <th class="col-md-7 text-right">@lang( 'purchase.estimated_amount' ):</th>
                    <td class="col-md-5 text-left">
                      <span id="estimated_amount" class="display_currency"></span>
                    </td>
                  </tr>
                </table>
              </div>
...
@section('javascript')
  ...
  <script type="text/javascript">
    $(document).ready( function(){
      ...
      // Auto-change status to 'received' when all purchase quantities equal received quantities
      function checkAndUpdateStatus() {
        var allQuantitiesMatch = true;
        var hasRows = false;

        $('#purchase_entry_table tbody tr').each(function() {
          var purchaseQty = parseFloat($(this).find('.purchase_quantity').val()) || 0;
          var receivedQty = parseFloat($(this).find('.asked_qty').val()) || 0;
          hasRows = true;

          if (purchaseQty !== receivedQty) {
            allQuantitiesMatch = false;
            return false; // break loop
          }
        });

        if (hasRows && allQuantitiesMatch) {
          var currentStatus = $('select[name="status"]').val();
          if (currentStatus === 'pending') {
            $('select[name="status"]').val('received').trigger('change');
          }
        }
      }

      // Monitor changes on quantity and asked_qty inputs
      $(document).on('change', '.purchase_quantity, .asked_qty', function() {
      ...
      // Initialize job sheet select2 with AJAX search
      $('#repair_job_sheet_id').select2({
        ajax: {
          url: '{{ action([\App\Http\Controllers\PurchaseController::class, "searchJobSheets"]) }}',
          dataType: 'json',
          delay: 250,
      ...
      @if(!empty($purchase->repair_job_sheet_id))
        // Pre-populate the select2 with existing value
        var jobSheetText = '{{ optional($purchase->job_sheet)->job_sheet_no ?? ("#" . $purchase->repair_job_sheet_id) }}';
        var option = new Option(jobSheetText, '{{ $purchase->repair_job_sheet_id }}', true, true);
        $('#repair_job_sheet_id').append(option).trigger('change');
      @endif
    });
  </script>
