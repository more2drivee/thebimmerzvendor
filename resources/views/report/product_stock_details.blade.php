                ...
                <div class="box-body">
                    ...
                    @if(!empty($stock_details))
                        ...
                        <div class="row">
                            ...
                            <table class="table" id="stock-mismatch-table">
                                <tr>
                                    <th>
                                        Product
                                    </th>
                            ...
                            @foreach($stock_details as $row)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{action([\App\Http\Controllers\ReportController::class, 'adjustProductStock'])}}?location_id={{$location->id}}&variation_id={{$row->variation_id}}&stock={{$row->total_stock_calculated}}" class="btn btn-primary btn-fix">Fix</a>
                                    </td>
                                </tr>
                            @endforeach
                            </table>
    ...
    <script type="text/javascript">
        $(document).ready( function () {
            ...
            $('#fix-all-btn').on('click', function() {
                var rows = $('#stock-mismatch-table tbody tr');
                if (rows.length === 0) {
                    toastr.warning('No stock mismatches to fix.');
                    return;
                }

                if (!confirm('Are you sure you want to fix all stock mismatches? This will update ' + rows.length + ' product(s).')) {
                    return;
                }

                var fixLinks = [];
                rows.each(function() {
                    var fixLink = $(this).find('a.btn-fix').attr('href');
                    if (fixLink) {
                        fixLinks.push(fixLink);
                    }
                });

                if (fixLinks.length === 0) {
                    toastr.error('No fix links found.');
                    return;
                }
                ...
                btn.html('<i class="fa fa-spinner fa-spin"></i> Fixing...');

                // Process all fixes sequentially
                var index = 0;
                var processNext = function() {
                    if (index < fixLinks.length) {
                        $.ajax({
                            url: fixLinks[index],
                            type: 'GET',
                            ...
                            error: function(xhr) {
                                toastr.error('Error fixing product ' + (index + 1) + ' of ' + fixLinks.length);
                                btn.prop('disabled', false);
                                btn.html('<i class="fa fa-check-circle"></i> Fix All');
                            }
                        });
                    } else {
                        // All done
                        toastr.success('All stock mismatches fixed successfully!');
                        setTimeout(function() {
                            location.reload();
                        }, 1500);
                    }
