<table class="table @if(!empty($for_ledger)) table-slim mb-0 bg-light-gray @else bg-gray @endif" @if(!empty($for_pdf)) style="width: 100%;" @endif>
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            <td>
                @php
                    $quantity = $sell_line->quantity ?? 1;
                    $disc_value = (float) ($sell_line->line_discount_amount ?? 0);
                    $disc_type = $sell_line->line_discount_type ?? 'fixed';
                    $line_discount_total = 0;

                    if ($disc_value > 0) {
                        if ($disc_type === 'percentage') {
                            $unit_base = $sell_line->unit_price_before_discount ?? $sell_line->unit_price ?? 0;
                            $line_discount_total = ($disc_value / 100) * $unit_base * $quantity;
                        } else {
                            // fixed stored as line total
                            $line_discount_total = $disc_value;
                        }
                    }
                @endphp
                @if(!empty($for_ledger))
                    @format_currency($line_discount_total)
                @else
                    <span class="display_currency" data-currency_symbol="true">{{ $line_discount_total }}</span>
                @endif
