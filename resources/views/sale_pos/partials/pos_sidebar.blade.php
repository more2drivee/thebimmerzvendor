...
<div class="row tw-mb-1">
                ...
                <div class="tw-dw-drawer-side" style="z-index: 4000">
                    ...
                    <div class="tw-dw-menu tw-w-2/4 tw-min-h-full tw-bg-white tw-p-6">
                        ...
                        <div class="row tw-mr-5">
                            <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-7 tw-w-auto  tw-h-auto tw-cursor-pointer  main-category-div main-category no-print"
                            ...
                            @foreach ($categories as $category)
                                    <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-7 tw-w-auto  tw-h-28  tw-cursor-pointer main-category-div  no-print"
                                ...
                                @if (!empty($category['sub_categories']))
                                    <div class="{{ $category['id'] }} all-sub-category" style="display: none">
                                        @foreach ($category['sub_categories'] as $sc)
                                            @if ($sc['parent_id'] != 0)
                                                <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-5 tw-w-auto tw-h-auto tw-cursor-pointer product_category no-print"
                                                    data-value="{{ $sc['id'] }}">
                                                    <div
                                                        class="tw-dw-card tw-w-25 tw-bg-base-100 tw-shadow-sm tw-h-auto tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-font-semibold tw-text-center tw-border-2">
                                                        <div class="tw-dw-card-body">
                    ...
                    <div class="tw-dw-menu tw-w-2/4 tw-min-h-full tw-bg-white tw-p-6">
                        ...
                        <div class="row tw-mr-5">
                            @foreach ($brands as $key => $brand)
                                <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-5 tw-w-auto tw-h-auto tw-cursor-pointer product_brand no-print"
                                    data-value="{{ $key }}">
                                    <div
                                        class="tw-dw-card tw-w-25 tw-bg-base-100 tw-shadow-sm tw-h-auto tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-font-semibold tw-text-center tw-border-2">
                                        <div class="tw-dw-card-body">
    ...
    @if (!empty($compat_brand_categories))
        <div class="col-sm-12 !tw-px-2" id="product_compatibility_div">
            <div class="tw-dw-drawer tw-dw-drawer-end">
                <input id="my-drawer-compat" type="checkbox" class="tw-dw-drawer-toggle">
                <div class="tw-dw-drawer-content">
                    <label for="my-drawer-compat"
                        class="tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 hover:tw-from-indigo-600 hover:tw-to-blue-600 focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-blue-500 focus:tw-ring-offset-2 active:tw-from-indigo-700 active:tw-to-blue-700 lg:tw-w-[98%] tw-w-full tw-flex tw-items-center tw-justify-center tw-gap-1 tw-text-base md:tw-text-lg tw-text-white tw-font-semibold tw-rounded-xl tw-h-12 tw-cursor-pointer">
                        <svg xmlns="http://www.w3.org/2000/svg" class="tw-w-5 icon icon-tabler icon-tabler-car"
                            width="44" height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff"
                            fill="none" stroke-linecap="round" stroke-linejoin="round">
                            <path stroke="none" d="M0 0h24v24H0z" fill="none" />
                        ...
                        @lang('lang_v1.compatibility')
                    </label>
                </div>
                <div class="tw-dw-drawer-side" style="z-index: 4000">
                    <label for="my-drawer-compat" aria-label="close sidebar"
                        class="tw-dw-drawer-overlay overlay-compat"></label>
                    <div class="tw-dw-menu tw-w-2/4 tw-min-h-full tw-bg-white tw-p-6">
                        <div class="tw-flex tw-items-center tw-mb-16">
                            <button type="button"
                                class="tw-dw-btn tw-dw-btn-accent compat-back tw-bg-transparent tw-border-2"
                                style="display: none">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                    class="tw-w-5 icon icon-tabler icon-tabler-chevron-left" width="44"
                                    height="44" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50"
                            ...
                            <h3 class="tw-text-center tw-flex-grow mx-auto compat_heading tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-inline-block tw-text-transparent tw-bg-clip-text tw-font-bold tw-text-base md:tw-text-2xl"
                                style="margin-bottom: 0px; margin-top:5px;">@lang('lang_v1.compatibility')</h3>
                            <button type="button" class="tw-dw-btn tw-dw-btn-error close-side-bar-compat">
                                <i class="fa fa-times-circle" aria-hidden="true"></i>
                            </button>
                        </div>

                        <div class="row tw-mr-5">
                            <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-7 tw-w-auto tw-h-auto tw-cursor-pointer main-compat-div no-print"
                                data-value="all" data-parent="0">
                                <div class="tw-dw-card tw-w-25 tw-bg-base-100 tw-shadow-sm tw-h-auto tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer !tw-text-xs md:!tw-text-sm tw-font-semibold tw-text-center tw-border-2">
                                    <div class="tw-dw-card-body">
                                        <h4 class="tw-flex tw-items-center tw-justify-center" style="margin-bottom: 0px; margin-top:0px; font-size: inherit; font-weight: inherit;">@lang('lang_v1.all')</h4>
                                    </div>
                                </div>
                            </div>

                            @foreach ($compat_brand_categories as $bc)
                                <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-7 tw-w-auto tw-h-28 tw-cursor-pointer main-compat-div no-print"
                                    data-value="{{ $bc['id'] }}" data-name="{{ $bc['name'] }}" data-parent="1">
                                    <div class="tw-dw-card tw-w-25 tw-bg-base-100 tw-shadow-sm tw-h-auto tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-font-semibold !tw-text-center tw-border-2">
                                        <div class="tw-dw-card-body" style="margin-bottom: -20px">
                                            <h4 class="tw-flex tw-items-center tw-justify-center" style="margin-bottom: 0px; margin-top:0px; font-size: inherit; font-weight: inherit;">{{ $bc['name'] }}</h4>
                                        </div>
                                        <div class="tw-dw-card-actions tw-justify-center">
                                            <button type="button" class="tw-dw-btn tw-dw-btn-accent tw-dw-btn-outline tw-dw-btn-sm compat_brand_category tw-mb-2" data-value="{{ $bc['id'] }}" data-parent="0">{{ __('lang_v1.all') }}</button>
                                            @if (!empty($bc['models']))
                                                <button type="button" class="tw-dw-btn tw-dw-btn-primary tw-dw-btn-outline tw-dw-btn-sm compat_brand_category tw-mb-2" data-parent="1" data-value="{{ $bc['id'] }}" data-name="{{ $bc['name'] }}">@lang('pagination.next')</button>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            @foreach ($compat_brand_categories as $bc)
                                @if (!empty($bc['models']))
                                    <div class="{{ $bc['id'] }} all-compat-models" style="display: none">
                                        @foreach ($bc['models'] as $m)
                                            <div class="col-lg-3 col-sm-4 col-xs-12 tw-mb-5 tw-w-auto tw-h-auto tw-cursor-pointer compat_model no-print" data-brand-category-id="{{ $bc['id'] }}" data-value="{{ $m['id'] }}">
                                                <div class="tw-dw-card tw-w-25 tw-bg-base-100 tw-shadow-sm tw-h-auto tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-text-xs md:tw-text-sm tw-font-semibold tw-text-center tw-border-2">
                                                    <div class="tw-dw-card-body">
                                                        <h4 class="tw-flex tw-items-center tw-justify-center" style="margin-bottom: 0px; margin-top:0px; font-size: inherit; font-weight: inherit;">{{ $m['name'] }}</h4>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
