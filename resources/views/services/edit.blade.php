<div class="modal-dialog" role="document">
    <div class="modal-content">
        <form method="post" action="{{ action([\App\Http\Controllers\ServiceController::class, 'update'], $service->id) }}" id="service_form">
            @csrf
            @method('PUT')
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="gridSystemModalLabel">@lang('Edit Labour')</h4>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="service_name" name="name" value="{{ old('name', $service->name) }}" required>
                        </div>
                    </div>
                </div>
                            ...
                            <select class="form-control select2" id="product_locations" name="product_locations[]" multiple>
                                @if(isset($business_locations) && count($business_locations) > 0)
                                    @foreach($business_locations as $id => $name)
                                        <option value="{{$id}}" {{ in_array($id, (array)old('product_locations', $assigned_location_ids ?? [])) ? 'selected' : '' }}>{{$name}}</option>
                                    @endforeach
                                @endif
                            </select>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="workshop_ids" name="workshop_ids[]" multiple data-selected-workshops="{{ implode(',', (array)old('workshop_ids', $assigned_workshop_ids ?? [])) }}">
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="price_type" name="price_type" required>
                                <option value="manual" {{ old('price_type', $service->product_custom_field1) === 'manual' ? 'selected' : '' }}>@lang('Price') (Manual)</option>
                                <option value="per_hour" {{ old('price_type', $service->product_custom_field1) === 'per_hour' ? 'selected' : '' }}>@lang('Per Hour')</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="row" id="flat_rate_group" style="display: none;">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="flat_rate_id">@lang('Labour Flat Rate')</label>
                            <select class="form-control select2" id="flat_rate_id" name="flat_rate_id" data-selected-flat-rate="{{ old('flat_rate_id', $service->product_custom_field2) }}">
                    ...
                    <div class="col-md-6" id="service_hours_group" style="display: none;">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="service_hours" name="service_hours" 
                                   step="0.1" min="0" value="{{ old('service_hours', $service->serviceHours) }}">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="service_price" name="price" 
                                   step="0.01" min="0" value="{{ old('price', $service->selling_price ?? optional($service->variations->first())->default_sell_price) }}" required>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="checkbox">
                                <label>
                                    <input type="checkbox" id="is_external" name="is_external" value="1"
                                           {{ !empty($service->is_external) ? 'checked' : '' }}>
                                    @lang('External Labor')
                                </label>
                            </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="prediction_km_interval" name="prediction_km_interval" 
                                   step="1" min="0" placeholder="e.g. 5000" value="{{ old('prediction_km_interval', $service->product_custom_field3) }}">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="prediction_time_interval" name="prediction_time_interval" 
                                   step="1" min="0" placeholder="e.g. 6" value="{{ old('prediction_time_interval', $service->product_custom_field4) }}">
                ...
                <div class="row">
                    <div class="col-md-12">
                        <small class="text-muted">
                            ...
                            @lang('Note: Only name and price fields are editable. Other service properties maintain system defaults.')
                        </small>
                    </div>
                </div>
            </div>
