...
@section('title', __('bundles.edit_bundle_sell_title'))

@section('content')
<section class="content-header">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">@lang('bundles.edit_bundle_sell_title')</h1>
</section>
	...
	<section class="content no-print">
		...
		{!! Form::open(['url' => route('bundles.sell.update', ['id' => $transaction->id]), 'method' => 'post', 'id' => 'bundle_quick_sell_form', 'files' => true]) !!}
					...
					<div class="row">
						...
						<div class="col-md-3">
							<div class="form-group">
								...
								{!! Form::select('status', $statuses, $transaction->status, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']) !!}
							</div>
						</div>
					...
					<div class="row col-sm-12 pos_product_div" style="min-height: 0">
						...
						<table class="table table-condensed table-bordered table-striped table-responsive" id="bundle_quick_sell_lines">
							...
							<tbody>
								...
								<tr>
									<td>
										<select name="lines[{{ $loop->index }}][bundle_id]" class="form-control line-bundle select2-bundle" style="width: 100%" required>
											<option value="">@lang('messages.please_select')</option>
											@if($bundle)
												<option value="{{ $bundle->id }}" selected>{{ $bundle->reference_no }}</option>
											@endif
										</select>
									</td>
									<td>
										<input type="text" name="lines[{{ $loop->index }}][name]" class="form-control" placeholder="Product name" value="{{ $line->product->name ?? '' }}" required>
									</td>
									<td>
										<input type="text" name="lines[{{ $loop->index }}][qty]" class="form-control input_number line-qty" placeholder="Qty" value="{{ $line->quantity }}" required>
									</td>
									<td>
										<input type="text" name="lines[{{ $loop->index }}][price]" class="form-control input_number line-price" placeholder="Price" value="{{ $line->unit_price_inc_tax }}" required>
									</td>
									<td>
										<input type="text" name="lines[{{ $loop->index }}][discount]" class="form-control input_number line-discount" placeholder="0" value="0">
									</td>
									<td class="text-right">
										<span class="display_currency line-subtotal" data-currency_symbol="true">{{ $line->line_total }}</span>
									</td>
						...
						<table class="table table-condensed table-bordered table-striped">
							<tr>
								<td>
									...
									<b>@lang('sale.item'):</b> 
									<span class="total_quantity">{{ $transaction->sell_lines->sum('quantity') }}</span>
									&nbsp;&nbsp;&nbsp;&nbsp;
									<b>@lang('sale.total'): </b>
										<span class="price_total">{{ $transaction->total_before_tax }}</span>
									</div>
								</td>
							</tr>
						</table>
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							<div class="input-group">
								...
								{!! Form::select('discount_type', ['fixed' => __('lang_v1.fixed'), 'percentage' => __('lang_v1.percentage')], $transaction->discount_type, ['class' => 'form-control', 'placeholder' => __('messages.please_select'), 'required']) !!}
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group">
							...
							<div class="input-group">
								...
								{!! Form::text('discount_amount', $transaction->discount_amount, ['class' => 'form-control input_number', 'required']) !!}
							</div>
						</div>
					</div>
					<div class="col-md-4"><br>
						<b>@lang('sale.discount_amount'):</b>(-) 
						<span class="display_currency" id="total_discount">{{ $transaction->discount_amount }}</span>
					</div>
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							<div class="input-group">
								...
								{!! Form::select('tax_rate_id', $taxes['tax_rates'], $transaction->tax_id, ['placeholder' => __('messages.please_select'), 'class' => 'form-control', 'required'], $taxes['attributes']) !!}
							</div>
						</div>
					</div>
				...
				@component('components.widget', ['class' => 'box-solid', 'title' => __('purchase.add_payment')])
					...
					<div class="row">
						<div class="col-md-4 col-md-offset-8">
							...
							<span id="total_payable">{{ $transaction->final_total }}</span>
						</div>
					</div>
				@endcomponent
...
<script>
    $(document).ready(function() {
        let lineIndex = {{ count($transaction->sell_lines) }};
        ...
        $('#add_quick_sell_line').on('click', function() {
            const rowHtml = `
                <tr>
                    <td>
                        <select name="lines[${lineIndex}][bundle_id]" class="form-control line-bundle select2-bundle" style="width: 100%" required>
                            <option value="">@lang("messages.please_select")</option>
                        </select>
                    </td>
                    <td>
                        <input type="text" name="lines[${lineIndex}][name]" class="form-control" placeholder="Product name" required>
                    </td>
                    <td>
                        <input type="text" name="lines[${lineIndex}][qty]" class="form-control input_number line-qty" placeholder="Qty" required>
                    </td>
                    <td>
                        <input type="text" name="lines[${lineIndex}][price]" class="form-control input_number line-price" placeholder="Price" required>
                    </td>
                    <td>
                        <input type="text" name="lines[${lineIndex}][discount]" class="form-control input_number line-discount" placeholder="0">
                    </td>
                    <td class="text-right">
            ...
            $('select[name="lines[' + lineIndex + '][bundle_id]"]').select2({
                ajax: {
        ...
        // Initialize Select2 for existing bundle dropdowns
        $('.select2-bundle').select2({
            ajax: {
                url: '{{ route("bundles.ajax.search") }}',
                dataType: 'json',
                delay: 250,
        ...
        $('#customer_id').select2({
            ajax: {
                url: '/contacts/get-contact-by-id',
                dataType: 'json',
                delay: 250,
                data: function(params) {
                    return {
        ...
        // Load initial customer if exists
        var defaultCustomerId = $('#default_customer_id').val();
        if (defaultCustomerId) {
            var defaultCustomerName = $('#default_customer_name').val();
            $('#customer_id').append(new Option(defaultCustomerName, defaultCustomerId, true, true)).trigger('change');
        }
