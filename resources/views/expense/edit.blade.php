      ...
      <div class="row">
        ...
        <div class="col-sm-4">
          <div class="form-group">
            ...
            {!! Form::select('invoice_ref', [], $expense->invoice_ref ?? null, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'id' => 'invoice_ref']); !!}
          </div>
        </div>
        <div class="col-sm-4">
          <div class="form-group">
...
<script type="text/javascript">
  ...
  $(function() {
    var $select = $('#invoice_ref');
    @if(!empty($related_txn))
      var txt = '';
      @php
        $parts = [];
        if (!empty($related_txn->invoice_no)) { $parts[] = '#'.$related_txn->invoice_no; }
        if (!empty(optional($related_txn->job_sheet)->job_sheet_no)) { $parts[] = 'JS: '.optional($related_txn->job_sheet)->job_sheet_no; }
        $text = implode(' - ', $parts) ?: ('#'.$related_txn->id);
      @endphp
      txt = @json($text);
      var option = new Option(txt, {{ $related_txn->id }}, true, true);
