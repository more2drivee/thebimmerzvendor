@extends('layouts.app')
@section('title', __('bundles.overview.transactions_table'))

@section('content')

    <!-- Content Header (Page header) -->
    <section class="content-header no-print">
        <h1  class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">@lang('bundles.overview.transactions_table')</h1>
    </section>
    ...
    <section class="content no-print">
        @component('components.filters', ['title' => __('report.filters')])
            ...
            @endif
        @endcomponent
        @component('components.widget', ['class' => 'box-primary', 'title' => __('bundles.overview.transactions_table')])
            @can('direct_sell.access')
                @slot('tool')
                    <div class="box-tools">
                        <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right"
                            href="{{ route('bundles.quick_sell.form') }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                                ...
                                <path d="M12 5l0 14" />
                                <path d="M5 12l14 0" />
                            </svg> @lang('bundles.quick_sell_action')
                        </a>
                    </div>
                @endslot
            @endcan
            @if (auth()->user()->can('direct_sell.view') ||
                ...
                @endphp
                <table class="table table-bordered table-striped ajax_view" id="bundle_sell_table">
                    <thead>
                        <tr>
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            bundle_sell_table = $('#bundle_sell_table').DataTable({
                processing: true,
                serverSide: true,
                fixedHeader:false,
                ...
                "fnDrawCallback": function(oSettings) {
                    __currency_convert_recursively($('#bundle_sell_table'));
                },
                "footerCallback": function(row, data, start, end, display) {
                    ...
                    $('.payment_method_count').html(__sum_payment_methods(data, 'payment_methods'));
                },
            ...
            $(document).on('click', 'a.delete-sale', function(e) {
                e.preventDefault();
                ...
                // First stage: soft delete (no password required)
                if (!isDeleted) {
                    swal({
                    ...
                    // Second stage: hard delete, only for admins and requires admin password
                    if (!isAdmin) {
