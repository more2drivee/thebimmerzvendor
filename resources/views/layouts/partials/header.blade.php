...
<div
    class="  tw-transition-all tw-duration-5000 tw-border-b   tw-from-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-800 tw-to-@if(!empty(session('business.theme_color'))){{session('business.theme_color')}}@else{{'primary'}}@endif-900 tw-shrink-0 lg:tw-h-15 tw-border-primary-500/30 no-print">
    <div class="tw-px-5 tw-py-3 h-100 d-flex align-items-center w-100 h-100 justify-content-between">
        <div class="tw-flex tw-items-start tw-justify-between tw-gap-6 w-100 d-flex align-items-center justify-content-between h-100 lg:tw-items-center nav-contain" >
            <a href="{{route('home_page')}}">
                @if(isset($admin_settings['login_logo']) && file_exists(public_path('storage/' . $admin_settings['login_logo'])))
                    <img src="{{ asset('storage/' . $admin_settings['login_logo']) }}" alt="Logo" class="logo">
                @else
...
{{--                </details>--}}
        ...
        class="d-none d-md-inline-flex align-items-center justify-content-center"
        style="
            border-radius:15px; 
            width:48px; 
            height:48px; 
        ...
        >
    <span class="visually-hidden" aria-hidden="true">Calculator</span>
    <i class="fas fa-calculator" style="font-size:1.6rem; color:var(--bs-primary);"></i>
</button> -->
...
{{--                        </a>--}}
        ...
        class="d-none d-sm-inline-flex align-items-center justify-content-center"
        style="
            border-radius: 15px;
            width:48px; 
            height:48px;
        ...
        "
>
    <span class="visually-hidden">Today's Profit</span>
    <i class="fas fa-chart-line"  style="font-size:1.6rem; color:var(--bs-primary);"></i>
</button>
                @endcan
       ...
       <div class="dropdown" style="border-radius:15px;">
    <button
        class="btn p-2 d-flex align-items-center gap-3 rounded-3 border-0 dropdown-toggle"
        type="button"
        data-toggle="dropdown"
        aria-haspopup="true"
        aria-expanded="false"
        style="
            background: linear-gradient(to right, rgba(13,110,253,0.1), rgba(59,130,246,0.1));
    ...
    >
        <div class="d-flex align-items-center justify-content-center"
             ...
             border-radius:50%;
                    background: linear-gradient(to bottom right, var(--bs-primary), #2563EB);">
