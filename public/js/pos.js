...
// compatibility global filters
var global_compat_brand_category_id = null;
var global_compat_model_id = null;
...
// on click sub category in category drawer
    ...
    // Compatibility drawer: brand category and model filters
    // Navigate to models list or apply brand category filter
    $(document).on('click', '.compat_brand_category', function() {
        var parent = $(this).data('parent');
        var brandCatId = $(this).data('value');
        $('input#suggestion_page').val(1);
        if (parent == 0) {
            // Apply filter: All under brand category (or clear if 'all')
            if (brandCatId === 'all') {
                global_compat_brand_category_id = null;
                global_compat_model_id = null;
            } else {
                global_compat_brand_category_id = brandCatId;
                global_compat_model_id = null;
            }
            var location_id = $('input#location_id').val();
            if (location_id != '' || location_id != undefined) {
            ...
            get_featured_products();
            $('.overlay-compat').trigger('click');
        } else {
            // Show models under this brand category
            var main_id = brandCatId;
            $('.main-compat-div').hide();
            $('.' + main_id + '.all-compat-models').fadeIn();
            $('.compat_heading').text('Models ' + ($(this).data('name') || ''));
            $('.compat-back').fadeIn();
        }
    });

    // Back to brand categories list
    $(document).on('click', '.compat-back', function(){
        $('.main-compat-div').fadeIn();
        $('.all-compat-models').hide();
        $('.compat-back').hide();
        $('.compat_heading').text('Compatibility');
    });

    // Apply model-specific filter
    $(document).on('click', '.compat_model', function(){
        var brandCatId = $(this).data('brand-category-id');
        var modelId = $(this).data('value');
        global_compat_brand_category_id = brandCatId || null;
        global_compat_model_id = modelId || null;
        $('input#suggestion_page').val(1);
        var location_id = $('input#location_id').val();
        if (location_id != '' || location_id != undefined) {
            get_product_suggestion_list(
    ...
    // Close side bar for compatibility
    $(document).on('click', '.close-side-bar-compat', function() {
        $('.overlay-compat').trigger('click');
    });
    ...
    $('#exchange_rate').change(function() {
        ...
        $('span#total_payable, span.total_payable_display').text(__currency_trans_from_en(shown_total, false));
    });
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null, compat_brand_category_id = null, compat_model_id = null) {
    ...
    $.ajax({
        ...
        data: {
            ...
            compat_brand_category_id: compat_brand_category_id,
...
function reset_pos_form(){
	...
	$('span.total_quantity, span.price_total, span#total_discount, span#order_tax, span#total_payable, span.total_payable_display, span#shipping_charges_amount').text(0);
...
function pos_print(receipt) {
    ...
    if (receipt.print_type == 'printer') {
        ...
        // If this is a transaction receipt, use the clean invoice URL
        if (receipt.transaction_id) {
            var cleanInvoiceUrl = '/sells/' + receipt.transaction_id + '/print-clean';
            window.open(cleanInvoiceUrl, '_blank');
            return;
        }
...
// Handle add expense category button click in modal
$(document).on('click', '#add_expense_category_btn_modal', function(e) {
    e.preventDefault();
    $.ajax({
        url: '/expense-categories/create',
        dataType: 'html',
        success: function(result) {
            // Create a nested modal for expense category
            var categoryModal = $('<div class="modal fade expense_category_nested_modal" tabindex="-1" role="dialog"></div>');
...
// Handle expense category form submission in nested modal
$(document).on('submit', '.expense_category_nested_modal form#expense_category_add_form', function(e) {
    e.preventDefault();
    var form = $(this);
    var data = form.serialize();
    ...
    $.ajax({
        ...
        success: function(result) {
            if (result.success === true) {
                $('.expense_category_nested_modal').modal('hide');

                // Add new option to expense category dropdown in the main modal
                var newOption = new Option(result.data.name, result.data.id, true, true);
                $('#expense_modal #expense_category_id').append(newOption).trigger('change');
