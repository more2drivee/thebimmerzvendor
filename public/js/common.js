...
if (typeof window.forceFocusFn !== 'function') {
    window.forceFocusFn = function () {
        var searchInput = document.querySelector('.select2-container--open .select2-search__field');
        if (searchInput) searchInput.focus();
    };
}

if (!window.__select2ForceFocusBound) {
    $(document).on('select2:open', () => {
        setTimeout(() => window.forceFocusFn(), 200);
    });
    window.__select2ForceFocusBound = true;
}
