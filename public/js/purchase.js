$(document).ready(function() {
    ...
    $(document).on('change', '.purchase_quantity', function() {
        ...
        //Auto-fill received_qty with quantity value
        var received_qty_input = row.find('input.received_qty');
        if (received_qty_input.length > 0) {
            var current_received_qty = __read_number(received_qty_input, true);
            //Only auto-fill if received_qty is empty or equal to previous quantity
            if (current_received_qty === 0 || current_received_qty === undefined || isNaN(current_received_qty)) {
                __write_number(received_qty_input, quantity, true);
            }
        }
    ...
    if ($('#purchase_table').length > 0) {
        ...
        columns: [
            ...
            { data: 'job_sheet_no', name: 'job_sheet_no', orderable: true, searchable: true },
            { data: 'location_name', name: 'location_name' },
            { data: 'name', name: 'name' },
            { data: 'status', name: 'status' },
            { data: 'payment_status', name: 'payment_status' },
            { data: 'final_total', name: 'final_total' },
            { data: 'received_qty_progress', name: 'received_qty_progress', orderable: false, searchable: false },
...
function update_table_total() {
    ...
    $('#purchase_entry_table tbody')
        ...
        .each(function() {
            ...
            var asked_qty = __read_number($(this).find('.asked_qty'), true);
            var unit_cost_after_tax = __read_number($(this).find('.purchase_unit_cost_after_tax'), true);
            estimated_amount += asked_qty * unit_cost_after_tax;
        });
    ...
    $('#estimated_amount').text(__currency_trans_from_en(estimated_amount, true, true));
}
...
$(document).on('click', '.sync_asked_qty', function() {
    $('#purchase_entry_table tbody tr').each(function() {
        var row = $(this);
        var quantity = __read_number(row.find('.purchase_quantity'), true);
        __write_number(row.find('.asked_qty'), quantity, true);
    });
    update_table_total();
    toastr.success(__('purchase.asked_qty_synced'));
});

$(document).on('change', '.asked_qty', function() {
    update_table_total();
});
    ...
    $("div#import_product_dz").dropzone({
        ...
        success: function(file, response) {
            if (response.success) {
                ...
                // Display import summary if skipped rows exist
                if (response.skipped_count > 0) {
                    var skippedDetails = '<div class="alert alert-warning mt-2"><strong>Import Summary:</strong><br>';
                    skippedDetails += 'Imported: ' + response.imported_count + '<br>';
                    skippedDetails += 'Skipped: ' + response.skipped_count + '<br>';
                    if (response.skipped_rows && response.skipped_rows.length > 0) {
                        skippedDetails += '<details class="mt-2"><summary>View skipped rows</summary><ul class="mb-0">';
                        response.skipped_rows.forEach(function(row) {
                            skippedDetails += '<li>Row ' + row.row + ' (SKU: ' + (row.sku || 'N/A') + '): ' + row.reason + '</li>';
                        });
                        skippedDetails += '</ul></details>';
                    }
                    skippedDetails += '</div>';
                    $('#import_purchase_products_modal .modal-body').append(skippedDetails);
                }

                this.removeAllFiles();

                // Only hide modal if no skipped rows to allow user to see the summary
                if (response.skipped_count === 0) {
