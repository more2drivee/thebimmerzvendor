$(document).ready(function() {
    ...
    $(document).on('click', '.merge-contact-action', function(e) {
        e.preventDefault();
        var contact_id = $(this).data('contact-id');
        var contact_name = $(this).data('contact-name');

        var html = `
            <div class="modal fade" id="merge_contact_modal" tabindex="-1" role="dialog">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal">&times;</button>
                            <h4 class="modal-title">${LANG.merge_contacts || 'Merge Contacts'}</h4>
                        </div>
                        <div class="modal-body">
                            <div class="alert alert-info">
                                ${LANG.merge_into_target || 'Selected contacts will be merged into'} <strong>${contact_name}</strong>
                            </div>
                            <div id="similar-contacts-section" style="display: none;">
                                <label>${LANG.similar_contacts || 'Similar Contacts'}:</label>
                                <div id="similar-contacts-list" style="margin-bottom: 15px; max-height: 200px; overflow-y: auto;"></div>
                            </div>
                            <div class="form-group">
                                <label>${LANG.select_contacts_to_merge || 'Select Contacts To Merge'}:</label>
                                <select id="source_contact_ids" class="form-control" style="width: 100%" multiple="multiple"></select>
                            </div>
                            <input type="hidden" id="target_contact_id" value="${contact_id}">
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">${LANG.close || 'Close'}</button>
                            <button type="button" class="btn btn-primary" id="confirm_merge">${LANG.merge || 'Merge'}</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        $('body').append(html);
        var $modal = $('#merge_contact_modal');
        $modal.modal('show');

        // Initialize Select2 with AJAX search on /customers/search
        var $sourceSelect = $modal.find('#source_contact_ids');
        $sourceSelect.select2({
            dropdownParent: $modal,
            placeholder: LANG.please_select || 'Please Select',
            width: 'resolve',
            ajax: {
                ...
                processResults: function(data) {
                    var results = $.map(data, function(item) {
                        // Exclude the target contact itself from source options
                        if (parseInt(item.id) === parseInt(contact_id)) {
                            return null;
                        }
        ...
        // Fetch and display similar contacts
        $.ajax({
            url: '/contacts/similar',
            method: 'GET',
            dataType: 'json',
            data: {
                contact_id: contact_id
            },
            success: function(similarContacts) {
                if (similarContacts.length > 0) {
                    var $similarSection = $modal.find('#similar-contacts-section');
                    var $similarList = $modal.find('#similar-contacts-list');

                    var html = '<div class="list-group">';
                    $.each(similarContacts, function(index, contact) {
                        var contactText = contact.name || '';
                        if (contact.mobile) {
                            contactText += ' - ' + contact.mobile;
                        }
                        html += `
                            <div class="list-group-item" style="display: flex; justify-content: space-between; align-items: center;">
                                <span>${contactText}</span>
                                <button type="button" class="btn btn-xs btn-success add-similar-contact" data-contact-id="${contact.id}" data-contact-text="${contactText.replace(/"/g, '&quot;')}">
                                    <i class="fa fa-plus"></i> ${LANG.add || 'Add'}
                                </button>
                            </div>
                        `;
                    });
        ...
        // Handle clicking "Add" on similar contacts
        $modal.on('click', '.add-similar-contact', function() {
            var contactId = $(this).data('contact-id');
            var contactText = $(this).data('contact-text');

            var option = new Option(contactText, contactId, true, true);
            $sourceSelect.append(option).trigger('change');

            $(this).closest('.list-group-item').fadeOut(300, function() {
                $(this).remove();
                if ($modal.find('#similar-contacts-list .list-group-item').length === 0) {
                    $modal.find('#similar-contacts-section').hide();
                }
            });
        });

        // Handle confirm merge click inside this modal only
        $modal.off('click', '#confirm_merge').on('click', '#confirm_merge', function() {
            var target_id = $modal.find('#target_contact_id').val();
            var source_ids = $sourceSelect.val() || [];

            if (!source_ids.length) {
                toastr.warning(LANG.please_select_contacts || 'Please select at least one contact to merge');
                return;
            }
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    var msg = (xhr.responseJSON && xhr.responseJSON.msg) ? xhr.responseJSON.msg : (LANG.something_went_wrong || 'Something went wrong');
                    toastr.error(msg);
                }
            });
        });
    ...
    if ($('form#bussiness_edit_form').length > 0) {
        ...
        // Apple .p8 files often have no mimetype; remove accept rule to prevent false validation errors
        var $appleKeyInput = $('input[name="apple_service_file"]');
        if ($appleKeyInput.length && $appleKeyInput.rules) {
            $appleKeyInput.rules('remove', 'accept');
        }
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        footerCallback: function ( row, data, start, end, display ) {
            ...
            if (json && json.payment_method_totals) {
                var payment_method_totals = json.payment_method_totals;
                var footer_html = '';

                // Define payment method translations
                var payment_method_names = {
                    'cash': 'Cash',
                    'card': 'Card',
                    'cheque': 'Cheque',
                ...
                for (var method in payment_method_totals) {
                    if (payment_method_totals.hasOwnProperty(method)) {
                        var method_name = payment_method_names[method] || method.charAt(0).toUpperCase() + method.slice(1).replace('_', ' ');
                        var amount = __currency_trans_from_en(payment_method_totals[method], true);
                        footer_html += '<strong>' + method_name + ':</strong> ' + amount + ' | ';
                    }
                }

                // Remove the last ' | '
                if (footer_html.length > 0) {
                    footer_html = footer_html.slice(0, -3);
                }

                $('.footer_payment_method_totals').html(footer_html);
            }
        },
    ...
    $(document).on('change', '.payment_types_dropdown', function() {
        ...
        $(this)
            ...
            .each(function() {
                if ($(this).attr('data-type') == payment_type && payment_type != 'card') {
                    to_show = $(this);
                } else {
    ...
    function reloadNotificationsList() {
        ...
        $.ajax({
            ...
            success: function(result) {
                var $items = $(result);
                var returnedCount = $items.filter('li.notification-li').not('.no-notification').length;

                $('li.notification-li').remove();
                $('ul#notifications_list').prepend($items);
                ...
                $('#notifications_select_all').prop('checked', false);
                $('input#notification_page').val(1);
    ...
    function refreshNotificationCount() {
        $('span.notifications_count').html(__fa_awesome());
        if (typeof getTotalUnreadNotifications === 'function') {
            getTotalUnreadNotifications();
        }
    }
    ...
    $(document).on('change', '#notifications_select_all', function() {
        var checked = $(this).is(':checked');
        $('ul#notifications_list .notification-select').prop('checked', checked);
    });

    $(document).on('click', '.mark_selected_notifications', function(e) {
        e.preventDefault();
        var ids = [];
        $('ul#notifications_list .notification-select:checked').each(function() {
            var id = $(this).val();
            if (id) ids.push(id);
        });
        ...
        $.ajax({
            method: 'POST',
            url: '/notifications/mark-selected',
            dataType: 'json',
    ...
    $(document).on('click', '.mark_all_notifications', function(e) {
        e.preventDefault();
        var token = $('meta[name="csrf-token"]').attr('content');
        if (!token) {
            console.error('CSRF token missing');
    ...
    types_of_service_table = $('#types_of_service_table').DataTable({
                        ...
                        columns: [
                            ...
                            { data: 'inspection_service', name: 'inspection_service' },
                            { data: 'action', name: 'action' },
                        ],
                        fnDrawCallback: function(oSettings) {
...
$(document).on('click', '.js-send-invoice-sms', function(e) {
    e.preventDefault();
    var $btn = $(this);
    var url = $btn.data('sms-url');
    if (!url) { return false; }
    var token = $('meta[name="csrf-token"]').attr('content');
    var $modalContent = $btn.closest('.modal-content');
    var $results = $modalContent.find('.js-sms-results');
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            if (!resp || resp.success !== true || !resp.sms) {
                toastr.error(typeof LANG !== 'undefined' && LANG.something_went_wrong ? LANG.something_went_wrong : 'Something went wrong');
                return;
            }

            var smsResults = resp.sms.results || [];
            if (!smsResults.length) {
                toastr.info(typeof LANG !== 'undefined' && LANG.no_sms_recipients ? LANG.no_sms_recipients : 'No SMS recipients');
                return;
            }

            var html = '<div class="tw-space-y-1"><div class="tw-text-sm tw-font-medium">SMS<\/div>';
            for (var i = 0; i < smsResults.length; i++) {
                var r = smsResults[i];
                var badge = r.success ? '<span class="tw-inline-block tw-px-2 tw-py-1 tw-rounded tw-bg-green-100 tw-text-green-700">Sent<\/span>' : '<span class="tw-inline-block tw-px-2 tw-py-1 tw-rounded tw-bg-red-100 tw-text-red-700">Failed<\/span>';
                html += '<div class="tw-flex tw-items-center tw-gap-2"><span class="tw-text-xs tw-text-gray-600">' + (r.recipient || '') + '<\/span>' + badge + '<\/div>';
            }
            html += '<\/div>';
...
// Share Invoice: Build modal with WhatsApp & SMS options
$(document).on('click', 'a.share_invoice', function(e) {
    e.preventDefault();
    var href = $(this).data('href');
    if (!href) { return false; }

    var smsUrl = $(this).data('sms-url') || '';

    var $container = $('div.view_modal');
    var token = $('meta[name="csrf-token"]').attr('content');
    var loadingHtml = '<div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button><h4 class="modal-title">' + (typeof LANG !== 'undefined' && LANG.view_invoice_url ? LANG.view_invoice_url : 'Invoice URL') + '</h4></div><div class="modal-body"><div class="tw-text-center tw-text-sm tw-text-gray-500">' + (typeof LANG !== 'undefined' && LANG.loading ? LANG.loading : 'Loading') + '...</div></div></div></div>';

    $container.html(loadingHtml).modal('show');
    if (!$container.attr('tabindex')) { $container.attr('tabindex', '-1'); }
    try { $container.trigger('focus'); } catch(e){}
    ...
    $.ajax({
        ...
        success: function(resp) {
            ...
            var url = resp.url || '';
            var waWeb = (resp.whatsapp && resp.whatsapp.web_url) ? resp.whatsapp.web_url : '';
            var waSingle = (resp.whatsapp && resp.whatsapp.single_url) ? resp.whatsapp.single_url : '';
            var hasRecipients = (resp.recipients && resp.recipients.length > 0);

            var waLink = waSingle || waWeb;
            var canSendSms = !!(smsUrl && resp.sms && resp.sms.can_send && hasRecipients);

            var buttonsHtml = '  <div class="tw-flex tw-gap-2 tw-flex-wrap">';
            if (waLink) {
                buttonsHtml += '    <a href="' + waLink + '" class="btn btn-success tw-inline-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-rounded tw-bg-green-600 hover:tw-bg-green-700 tw-text-white" target="_blank" rel="noopener" aria-label="WhatsApp" tabindex="0">' +
                    '      <i class="fab fa-whatsapp"><\/i> WhatsApp' +
                    '    <\/a>';
            }
            if (canSendSms) {
                buttonsHtml += '    <button type="button" class="tw-inline-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-rounded tw-bg-blue-600 hover:tw-bg-blue-700 tw-text-white js-send-invoice-sms" data-sms-url="' + smsUrl + '">' +
                    '      <i class="fa fa-comment"><\/i> ' + ((typeof LANG !== 'undefined' && LANG.send_sms) ? LANG.send_sms : 'Send SMS') +
                    '    <\/button>';
            }
            buttonsHtml += '  <\/div>';
            ...
            var body = ''+
                ...
                '  <div class="tw-break-words tw-text-sm"><strong>' + (typeof LANG !== 'undefined' && LANG.invoice_url ? LANG.invoice_url : 'Invoice URL') + ':<\/strong> <a href="' + url + '" target="_blank" rel="noopener">' + url + '<\/a><\/div>' +
                buttonsHtml +
                '  <div class="tw-mt-2 js-sms-results tw-space-y-1" style="display:none;"><\/div>' +
                (hasRecipients ? '<div class="tw-text-xs tw-text-gray-600">' + ((typeof LANG !== 'undefined' && LANG.recipients) ? LANG.recipients : 'Recipients') + ': ' + resp.recipients.join(', ') + '<\/div>' : '') +
                '<\/div>';

            var html = '<div class="modal-dialog" role="document"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;<\/span><\/button><h4 class="modal-title">' + (typeof LANG !== 'undefined' && LANG.share_invoice ? LANG.share_invoice : 'Share Invoice') + '<\/h4><\/div><div class="modal-body">' + body + '<\/div><\/div><\/div>';
            $container.html(html);
        },
        error: function() {
            toastr.error(typeof LANG !== 'undefined' && LANG.something_went_wrong ? LANG.something_went_wrong : 'Something went wrong');
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            if ($('li.no-notification').length == 0) {
                $('ul#notifications_list').append($items);
            }
    ...
    if ($('span.notifications_count').length) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                var total = parseInt(data.total_unread, 10) || 0;
                if (total > 0) {
                    $badge.text(total);
                } else {
                    $badge.text('');
