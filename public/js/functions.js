...
function __sum_payment_methods(data, key) {
    var payment_methods = [];
    for (var r in data){
        var element = $(data[r][key]);
        if (element.data('orig-value')) {
            var method_name = element.data('orig-value');
            ...
            if (!(method_name in payment_methods)) {
                ...
                payment_methods[method_name]['total'] = total_paid;
                payment_methods[method_name]['count'] = 1;
                payment_methods[method_name]['display_name'] = element.data('status-name');
            } else {
                payment_methods[method_name]['total'] += total_paid;
                payment_methods[method_name]['count'] += 1;
            }
    ...
    for (var key in payment_methods) {
        html += payment_methods[key]['display_name'] + ' - ' + payment_methods[key]['count'] +
                ' (' + __currency_trans_from_en(payment_methods[key]['total']) + ')</br>';
    }
